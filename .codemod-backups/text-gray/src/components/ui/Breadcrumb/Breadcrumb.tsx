...
const CollapsedIndicator: React.FC<{
  ...
  return (
    <div className="relative">
      ...
      {isExpanded && (
        <div className="absolute top-full left-0 mt-1 py-1 z-10 min-w-48 surface-card elevation-dropdown border-subtle rounded-md">
          {hiddenItems.map((item) => (
            <Button
              key={item.id}
              variant="ghost"
              size="xs"
              className="w-full justify-start px-3 py-2 h-auto text-gray-700 dark:text-gray-300 surface-subtle-hover dark:hover:bg-gray-700"
              onClick={() => {
                onItemClick(item);
                setIsExpanded(false);
              }}
