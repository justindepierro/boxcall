...
export const RouteToolbar: React.FC<RouteToolbarProps> = ({
  ...
  return (
    <div className="flex flex-col gap-3 px-5 py-4 bg-surface rounded-lg border border-border shadow-sm">
      {/* Row 1: Section Label, Drawing Tools, Route Type */}
      <div className="flex items-center gap-4">
        {/* Section Label */}
        <div className="flex items-center gap-2 text-xs text-content-secondary font-medium pr-4 border-r border-border">
        ...
        <div className="relative">
          <button
            ...
            className="px-4 py-2 rounded-full text-xs font-medium bg-surface-secondary hover:bg-surface-tertiary transition-colors flex items-center gap-2"
            title={currentTypeConfig.description}
          >
            <Icon name={currentTypeConfig.icon as any} size="sm" />
            <span
              className={`text-${currentTypeConfig.color}-600 font-semibold`}
            >
      ...
      {/* Row 2: Route Actions and Help Text */}
      <div className="flex items-center gap-4">
        {/* Route Actions */}
        <div className="flex items-center gap-2">
          ...
          {onClearAllRoutes && (
            <button
              ...
              className="px-4 py-2 rounded-full text-xs font-medium bg-surface-secondary hover:bg-surface-tertiary text-content-secondary transition-colors flex items-center gap-1.5"
        ...
        <div className="ml-auto flex-1 text-right">
          {activeTool === "draw-route" && (
            <div className="text-xs text-content-tertiary italic">
              Click a player to start, then click to add waypoints. Double-click
              to finish.
            </div>
          )}
          {activeTool === "edit-waypoint" && (
