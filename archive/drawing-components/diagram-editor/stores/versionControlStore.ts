...
export const useVersionControlStore = create<VersionControlState>((set, get) => ({
  ...
  createVersion: async (description?: string) => {
    ...
    try {
      ...
      const { data, error } = await (supabase as any).rpc("create_named_play_version", {
  ...
  rollbackToVersion: async (versionNumber: number) => {
    ...
    try {
      const { data, error } = await (supabase as any).rpc("rollback_play_to_version", {
        p_play_id: currentPlayId,
  ...
  setSelectedVersionId: (versionId: string | null) => set({ selectedVersionId: versionId }),
