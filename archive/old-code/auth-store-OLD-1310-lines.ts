    ...
    (set, get) => ({
      ...
      fetchUserProfile: async (userId: string) => {
        ...
        if (cached && now - cached.timestamp < cached.ttl) {
          console.log("👤 [fetchUserProfile] Cache hit, using cached profile");
        ...
        try {
          console.log("👤 [fetchUserProfile] Fetching via direct REST API...");
          debug("👤 [fetchUserProfile] Fetching via direct REST API...");

          // Get auth token from localStorage
          const authData = localStorage.getItem("boxcall-auth");
          console.log("👤 [fetchUserProfile] localStorage boxcall-auth exists:", !!authData);
          
          // Supabase stores session at root level: { access_token, refresh_token, user, ... }
          const parsedAuth = authData ? JSON.parse(authData) : null;
          const token = parsedAuth?.access_token;
          console.log("👤 [fetchUserProfile] Has access_token:", !!token);
...
const initializeAuth = async () => {
  ...
  try {
    ...
    // 🔧 SIMPLIFIED: Just use Supabase's getSession() - it reads from localStorage automatically
    // The Supabase client already handles localStorage caching internally
    // No need for manual localStorage parsing or custom cache layers
    console.log("🔐 [Auth] Getting session from Supabase client...");
    ...
    if (session) {
      ...
      useAuth.getState().fetchUserProfile(session.user.id).catch((err) => {
        logError("Error fetching profile:", err);
      });

      const elapsed = performance.now() - startTime;
      success(`Auth init complete: ${elapsed.toFixed(0)}ms`);
    } else {
