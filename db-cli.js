...
async function checkStatus() {
  ...
  try {
    ...
    const { data, error } = await supabase.from('plays').select('count', { count: 'exact', head: true });
...
function showHelp() {
  ...
  log('    Example: node db-cli.js migrate database/migrations/008_add_coverage_tracking.sql', 'dim');
  log('');
...
switch (command) {
  ...
  case 'migrate':
    if (!args[0]) {
      ...
      log('Example: node db-cli.js migrate database/migrations/008_add_coverage_tracking.sql\n', 'yellow');
      process.exit(1);
    }
    runMigration(args[0]);
    break;
  ...
  case 'query':
    if (!args[0]) {
      ...
      log('Example: node db-cli.js query "SELECT COUNT(*) FROM plays"\n', 'yellow');
      process.exit(1);
    }
    runQuery(args.join(' '));
    break;
