...
```ts
interface DiagramFieldSettings { variant: 'hs'|'college'|'pro'|'flag'; showYardLines: boolean; showHashMarks: boolean; showPlayerLabels: boolean; showRouteArrows: boolean; }
interface DiagramPlayerMeta { id: string; label: string; role?: string; side?: 'O'|'D'|'ST'; x: number; y: number; color?: string; locked?: boolean; }
interface DiagramDocumentV2 extends DiagramDocumentV1 { field: DiagramFieldSettings; }
```
