...
const { auditFormationDirections } = await import(
  "./src/utils/formationAudit.ts"
);
...
```javascript
const { data: formations } = await supabase.from("formations").select("*");

const leftFormations = formations?.filter((f) => f.direction === "left") || [];
const rightFormations =
  formations?.filter((f) => f.direction === "right") || [];
...
const standalone =
  formations?.filter((f) => f.opposite_formation_id === f.id) || [];

const needingOpposites =
...
```javascript
// In browser console
const { data: playbooks } = await supabase.from("playbooks").select("*");
