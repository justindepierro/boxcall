    ...
    "no-raw-tailwind-colors": {
      ...
      create(context) {
        ...
        const SEMANTIC_SUGGESTIONS = {
          ...
          "text-amber-300":
            "text-warning-400 (light mode) or keep with dark: prefix",
        ...
        const ARBITRARY_VALUE_SUGGESTIONS = {
          ...
          "text-[11px]":
            "text-xs (12px) - consider adjusting scale or accepting closest match",
        ...
        function reportMatches(value, node) {
          ...
          for (const match of colorScaleMatches) {
            ...
            const precedingText = value.substring(
            ...
            const suggestion =
              SEMANTIC_SUGGESTIONS[utility] ||
              "a semantic token (see design system docs)";
            context.report({
              node,
