    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      try {
        ...
        if (error) {
          ...
          const { error: sqlError } = await supabase.from('_migrations').insert({ statement });
...
if (!migrationPath) {
  ...
  log('Example: node migrate-cli.js database/migrations/008_add_coverage_tracking.sql\n', 'yellow');
  process.exit(1);
}
