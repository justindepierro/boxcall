...
export function CalendarPermissionsManager({
  ...
  const canManagePermissions = ["owner", "head_coach"].includes(
    currentUserRole
  );
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        <div className="flex items-center">
          ...
          <p className="text-sm text-yellow-700">
            You don't have permission to manage calendar permissions. Contact a
            team owner or head coach.
          </p>
        </div>
      </div>
    );
...
function RoleDescriptions() {
  const roles: Array<{
  ...
  }> = [
    {
      ...
      title: "Owner",
      description:
        "Full access to all calendar features and permissions management",
    ...
    <div className="bg-gray-50 rounded-lg p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {roles.map((roleInfo) => (
          ...
          >
            <div
              className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${roleInfo.color} mb-2`}
            >
              {roleInfo.title}
            </div>
...
function MemberPermissionRow({
  ...
  const { permissions, loading, updatePermissions, revokePermissions } =
    useCalendarPermissions(member.id, teamId);
  ...
  const handleRevoke = async () => {
    ...
    if (
      confirm(
        `Are you sure you want to revoke calendar permissions for ${member.name}?`
      )
    ) {
      setUpdating(true);
      try {
                ...
                className={`px-2 py-1 text-xs font-medium rounded-full ${
                  currentRole === "owner"
                    ...
                    : currentRole === "head_coach"
                      ...
                      : currentRole === "assistant_coach"
                        ? "bg-green-100 text-green-800"
                        : currentRole === "team_captain"
  ...
  return (
    <div className="space-y-4">
      <div className="text-sm text-gray-600">
        <p>
          <strong>Role:</strong> {permissions.role.replace("_", " ")}
        </p>
        <p>
          <strong>Granted by:</strong> {permissions.granted_by}
        </p>
        <p>
        ...
        {permissions.expires_at && (
          <p>
            ...
            {new Date(permissions.expires_at).toLocaleDateString()}
          </p>
        )}
      </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        value={role}
                        onChange={(e) =>
                          setRole(e.target.value as CalendarRole)
                        }
                        className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                      >
