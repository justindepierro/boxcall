...
async function addDemoTeam() {
  ...
  try {
    ...
    if (rlsError) {
      console.log('⚠️  RLS disable failed, trying direct insert with service role...');
    } else {
    ...
    if (teamError) {
      ...
      console.log('');
      console.log('💡 You may need to manually run this SQL in Supabase dashboard:');
      console.log('ALTER TABLE teams DISABLE ROW LEVEL SECURITY;');
      console.log('INSERT INTO teams (name, school_name, mascot, season_year) VALUES (\'Demo Team\', \'BoxCall High\', \'Eagles\', 2025);');
      return;
    }
