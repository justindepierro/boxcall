...
async function applyMigration() {
  console.log('🚀 Applying Migration 302: Add coaching and social media fields');
  ...
  try {
    ...
    for (const column of newColumns) {
      try {
        ...
        if (error && error.message?.includes('column') && error.message?.includes('does not exist')) {
          console.log(`    ❌ Column ${column.name} needs to be added manually`);
        } else {
    ...
    const { error: testError } = await supabase
      .from('profiles')
      .select('coaching_experience, education, social_twitter, personal_website')
      .limit(1);
