...
async function testKnownTables() {
  ...
  const anonClient = createClient(supabaseUrl, process.env.VITE_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imx2bXVpcXdpaGxwbndwcGRxcWZsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTIwMjIzNDgsImV4cCI6MjA2NzU5ODM0OH0.3SreGdPAJ2J5XcQVbNIbzK378j15ZJnwQqscBE2HkII', {
    auth: { autoRefreshToken: false, persistSession: false }
  });
    ...
    try {
      ...
      if (error) {
        if (error.code === '42501' || error.message.includes('permission denied')) {
          ...
          console.log(`${table.padEnd(30)}${'Not Found'.padEnd(15)}⚠️  Missing`);
          notFound++;
        } else {
          console.log(`${table.padEnd(30)}${'Error'.padEnd(15)}❓ ${error.code}`);
        }
      } else {
        // Check if we got any data (should be 0 for RLS-protected tables with no auth)
        if (count === 0 || count === null) {
          console.log(`${table.padEnd(30)}${'Empty/RLS'.padEnd(15)}✅ Likely Secure`);
          secure++;
        } else {
          console.log(`${table.padEnd(30)}${`${count} rows`.padEnd(15)}❌ EXPOSED!`);
          insecure++;
        }
      }
    } catch (e) {
      console.log(`${table.padEnd(30)}${'Exception'.padEnd(15)}❓ ${e.message}`);
    }
  }
