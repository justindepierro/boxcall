...
const variantCounts = {};
const variantRegex = /<Button[^>]*variant={(?:"([^"]+)"|'([^']+)')|<Button[^>]*variant=\"([^\"]+)\"/g;
...
writeFileSync(join(outDir, 'button-variant-histogram.json'), JSON.stringify(data, null, 2));
...
console.log('[button-variant-histogram] Generated counts for', Object.keys(variantCounts).length, 'variants');
