...
async function checkTables() {
  try {
    ...
    if (defError) {
      console.log('❌ achievement_definitions table not found:', defError.message);
      ...
      if (createError) {
        console.log('❌ Failed to create achievement_definitions:', createError.message);
      } else {
    ...
    if (progError) {
      console.log('❌ achievement_progress table not found:', progError.message);
    } else {
    ...
    if (achError) {
      ...
      achievements.forEach(ach => console.log(`   - ${ach.title}: ${ach.description}`));
    }
