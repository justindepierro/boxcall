...
async function checkDuplicateTeams() {
  ...
  try {
    ...
    const { data: teams, error } = await supabase
      .from("teams")
      .select(
        "id, name, school_name, school_district, school_city, school_state, created_at, status"
      )
    ...
    if (duplicateGroups.length === 0) {
      ...
      duplicateGroups.forEach((group, index) => {
        ...
        group.forEach((team, teamIndex) => {
          ...
          if (team.school_city && team.school_state) {
            console.log(
              `     Location: ${team.school_city}, ${team.school_state}`
            );
          }
      ...
      console.log(
        "4. Update team creation process to prevent future duplicates"
      );
    }
    ...
    teams.slice(0, 10).forEach((team, index) => {
      console.log(
        `${index + 1}. ${team.name} (${team.school_name}) - ${team.created_at}`
      );
    });
  } catch (error) {
...
function areTeamsSimilar(team1: TeamRecord, team2: TeamRecord): boolean {
  ...
  const schoolSimilarity = stringSimilarity(
    team1.school_name,
    team2.school_name
  );
  ...
  if (
    ...
    const cityMatch =
      stringSimilarity(team1.school_city, team2.school_city) > 0.8;
    const stateMatch =
      team1.school_state.toLowerCase() === team2.school_state.toLowerCase();
  ...
  if (team1.school_district && team2.school_district) {
    districtMatch =
      stringSimilarity(team1.school_district, team2.school_district) > 0.8;
  }
