...
async function checkFormations() {
  ...
  try {
    ...
    const { data: formations, error } = await supabase
      .from("formations")
      .select(
        "id, name, direction, opposite_formation_id, usage_count, playbook_id, creation_source, metadata_quality"
      )
    ...
    // Group by playbook
    const byPlaybook = formations.reduce(
      (acc, f) => {
...
function analyzePlaybook(formations: FormationData[]) {
  ...
  // Priority breakdown
  const highPriority = formations.filter(
    (f) =>
      f.direction &&
  ...
  const medPriority = formations.filter(
    (f) =>
      f.direction &&
      (!f.opposite_formation_id || f.opposite_formation_id === f.id) ===
        false &&
  ...
  const lowPriority = formations.filter(
    (f) =>
      f.direction &&
      (!f.opposite_formation_id || f.opposite_formation_id === f.id) ===
        false &&
  ...
  if (highPriority.length + medPriority.length + lowPriority.length > 0) {
    ...
    if (highPriority.length > 0) {
      ...
      highPriority.slice(0, 5).forEach((f) => {
        console.log(
          `      • ${f.name} (${f.direction}) - ${f.usage_count || 0} uses`
        );
      });
    ...
    if (lowPriority.length > 0) {
      ...
      lowPriority.slice(0, 5).forEach((f) => {
        console.log(
          `      • ${f.name} (${f.direction || "no direction"}) - ${f.usage_count || 0} uses`
        );
      });
  ...
  topFormations.forEach((f, i) => {
    ...
    console.log(
      `      Direction: ${f.direction || "none"} | Uses: ${f.usage_count || 0} | ${status}`
    );
  ...
  if (formations.some((f) => f.creation_source)) {
    const bySources = formations.reduce(
      (acc, f) => {
  ...
  if (formations.some((f) => f.metadata_quality)) {
    const byQuality = formations.reduce(
      (acc, f) => {
