...
const allTypographyImportsRE = /import\s+{[^}]*\bTypography\b[^}]*}\s+from\s+['"]([^'";]+design-system(?:\/Typography)?)['"];?/g;
...
for (const file of collectFiles(root)) {
  ...
  const normalizedPaths = [...new Set(imports.map(i => normalizePath(i.path)))];
  ...
  const newLines = lines.filter(line => {
    const importLineMatch = line.match(/import\s+{[^}]*\bTypography\b[^}]*}\s+from\s+['"][^'";]+design-system(?:\/Typography)?['"];?/);
  ...
  for (let i = 0; i < newLines.length; i++) {
    if (newLines[i].match(/import\s+{[^}]*\bTypography\b[^}]*}\s+from\s+['"][^'";]+design-system(?:\/Typography)?['"];?/)) {
  ...
  if (updated !== code) {
    ...
    changed.push({ file, removed: matchCount - 1, canonical: canonicalImportLine });
  }
}
...
if (changed.length === 0) {
  ...
  console.log(`${write ? 'Fixed' : 'Would fix'} duplicate Typography imports in ${changed.length} file(s).`);
