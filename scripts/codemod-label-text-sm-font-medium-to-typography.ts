...
interface Options { dryRun: boolean; root: string; }
...
function walk(dir: string): string[] {
  return readdirSync(dir, { withFileTypes: true }).flatMap(d => {
    if (d.name.startsWith('.') || d.name === 'node_modules' || d.name === 'dist' || d.name === 'coverage') return [];
...
function ensureImport(src: string): string {
  if (/Typography\s*from/.test(src) || /{\s*Typography\s*}/.test(src)) return src; // already imported
  const lines = src.split(/\n/);
  let lastImportIdx = -1;
  for (let i=0;i<lines.length;i++) if (/^import\s+/.test(lines[i])) lastImportIdx = i;
  const importStmt = 'import { Typography } from "@/components/design-system/Typography";';
  if (lastImportIdx >= 0) {
...
function transformFile(path: string) {
  ...
  if (!/text-sm\s+font-medium/.test(src) && !/font-medium\s+text-sm/.test(src)) return; 

  for (const tag of targetTags) {
    const blockRegex = new RegExp(`<${tag}([^>]*?)className=\\"([^\\"]*?(?:text-sm[^\\"]*font-medium|font-medium[^\\"]*text-sm)[^\\"]*)\\"([^>]*)>([\\s\\S]*?)</${tag}>`, 'g');
    src = src.replace(blockRegex, (match, beforeAttrs, classValue, afterAttrs, inner) => {
      ...
      const cleanedClasses = classValue
        ...
        .trim();
      localOccurrences++;
      const clsAttr = cleanedClasses.length ? ` className=\\"${cleanedClasses}\\"` : '';
  ...
  if (localOccurrences) {
    ...
    console.log(`${opts.dryRun ? 'Would transform' : 'Transformed'} ${path} (${localOccurrences} occurrences)`);
  }
}
