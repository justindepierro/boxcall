...
const inverseBgRegex = /(bg-(?:gray|slate|stone|zinc|neutral|brand-jade|jade|red|blue|navy|black)-(?:[5-9]00|[6-9]00)|bg-gray-800|bg-gray-900|bg-black)/;
...
function replacePairs(content){
  for (const rule of pairRules){
    ...
    for (const light of rule.lights){
      ...
      content = content.replace(classAttrRegex, (full, attrName, brace, quote, inner, endQuote) => {
...
function replaceInverse(content){
  const classAttrRegex = /(className|class)=(\{?)(["'`])([\s\S]*?)(\3)/g;
  return content.replace(classAttrRegex, (full, attrName, brace, quote, inner, endQuote) => {
...
async function run(){
  const files = await globby(['src/**/*.{ts,tsx}'], { gitignore: true, ignore: ['**/node_modules/**'] });
  const changed = [];
