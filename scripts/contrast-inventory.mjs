...
const MID_BG_WHITE_TEXT = /(bg-(?:red|yellow|orange|amber|emerald|green|lime|blue|indigo|purple|violet|pink|rose|jade|navy)-(?:500))[^\n]*text-white/g;
const LIGHT_BG_BUTTON_PATTERN = /<(Button|a)\b[^>]*className="[^"]*(bg-(?:[a-z-]+-(?:50|100|200)))[^"]*(?:\btext-[a-z-]+-700)?[^"]*"/g;
...
for (const file of files) {
  ...
  for (const w of whiteText) {
    if (lightBg.some(l => Math.abs(l.index - w.index) < 120)) whiteOnLightPairs++;
  }

  if (lightBg.length || whiteOnLightPairs || midTone.length || lightBgButtons.length) {
    results.push({
      file,
...
mdLines.push('|--------|-------|');
mdLines.push(`| Light background utility occurrences | ${summary.lightBgCount} |`);
mdLines.push(`| White text near light background (heuristic) | ${summary.whiteTextOnLight} |`);
mdLines.push(`| Mid-tone (500) bg with white text | ${summary.midToneWhite} |`);
mdLines.push(`| Light background button patterns | ${summary.lightBgButtons} |`);
mdLines.push('');
mdLines.push('## Files');
for (const r of results) {
  mdLines.push(`### ${r.file}`);
  if (r.lightBg.length) mdLines.push(`- Light BG samples: ${[...new Set(r.lightBg)].slice(0,8).join(', ')}`);
  if (r.whiteOnLightPairs) mdLines.push(`- White text near light bg pairs: ${r.whiteOnLightPairs}`);
  if (r.midToneWhite.length) mdLines.push(`- Mid-tone bg + white: ${r.midToneWhite.length}`);
  if (r.lightBgButtons.length) mdLines.push(`- Light background button patterns: ${r.lightBgButtons.length}`);
  mdLines.push('');
}
