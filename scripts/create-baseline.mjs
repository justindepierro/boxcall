...
function distSnapshot() {
  ...
  fs.writeFileSync(path.join(outDir, 'bundle-stats.json'), JSON.stringify(bundleReport, null, 2));
}
...
function styleSnapshot() {
  ...
  scan(path.join(process.cwd(), 'src'), (file, lines) => {
    ...
    if (hits.length) offenders.push({ file: path.relative(process.cwd(), file), lines: hits });
  });
  fs.writeFileSync(path.join(outDir, 'raw-gray-offenders.json'), JSON.stringify(offenders, null, 2));
}
...
function perfSnapshot() {
  ...
  fs.writeFileSync(path.join(outDir, 'performance-budgets.json'), JSON.stringify(perf, null, 2));
}
...
function scan(dir, onFile) {
  for (const e of fs.readdirSync(dir)) {
    ...
    if (stat.isDirectory()) scan(full, onFile); else if (/\.(tsx?|css|md)$/.test(e)) {
