...
async function createCorrectedDemoData() {
  try {
    console.log('🔧 Creating corrected demo data...');

    // Create admin profile (id is TEXT, not UUID)
    console.log('🚀 Creating admin profile...');
    const { data: _admin, error: adminError } = await supabase
      .from('profiles')
      .insert({
    ...
    // Create team membership for admin (using 'role' not 'team_role')
    ...
    // Create a simple team post (author_id is TEXT)
    ...
    console.log('🎉 Corrected demo data creation complete!');
