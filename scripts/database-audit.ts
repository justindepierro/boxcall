...
async function auditDatabase() {
  ...
  const tables = [
    'teams', 'team_members', 'profiles', 'playbooks', 'plays', 'play_calls',
    'team_posts', 'practice_schedules', 'game_plans', 'equipment',
    'game_results', 'calendar_events', 'practice_scripts', 'achievements'
  ];
  ...
  for (const table of tables) {
    try {
      ...
      console.log(`${serviceRlsStatus} ${table}: ${count || 0} records (Service: ${serviceRlsStatus}, Anon: ${anonRlsStatus})`);
