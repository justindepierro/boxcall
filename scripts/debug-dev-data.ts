  ...
  try {
    ...
    if (membershipError) {
      ...
      memberships?.forEach((membership) => {
        console.log(
          `  - Team: ${membership.team_id}, Role: ${membership.team_role}, Status: ${membership.status}`
        );
      });
    }
    ...
    if (allMembershipError) {
      ...
      allMemberships?.forEach((membership) => {
        console.log(
          `  - User: ${membership.user_id}, Team: ${membership.team_id}, Role: ${membership.team_role}`
        );
      });
    }
    ...
    if (roleServiceError) {
      ...
      console.log(
        `📊 Found ${roleServiceQuery?.length || 0} active memberships`
      );
    }
  } catch (error) {
