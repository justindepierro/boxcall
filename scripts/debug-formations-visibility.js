...
async function debugFormations() {
  ...
  if (allError) {
    ...
    allFormations?.forEach((f) => {
      console.log(
        `      - ${f.name} (${f.direction}) - created_by: ${f.created_by?.substring(0, 8)}...`
      );
    });
    console.log("");
  }
  ...
  if (policies) {
    ...
    console.log(
      "   ⚠️  Could not fetch policies (need to check Supabase dashboard)"
    );
  }
  console.log("");
  ...
  console.log(
    `   Playbook: ${playbook?.name} (${playbook?.id?.substring(0, 8)}...)`
  );
  ...
  if (membership && membership.length > 0) {
    console.log(
      `   ✅ You ARE a member: ${membership[0].team_role} (${membership[0].status})`
    );
  } else {
  ...
  // Sign in as user
  const { data: session } = await userSupabase.auth
    .signInWithPassword({
  ...
  console.log(
    `   Session: ${session ? "Authenticated" : "Not authenticated (expected)"}`
  );
  ...
  if (userError) {
    ...
    console.log(
      `   ❌ This is likely the problem! RLS is blocking unauthenticated queries.\n`
    );
  } else {
    console.log(
      `   ✅ User can see ${userFormations?.length || 0} formations\n`
    );
  }
