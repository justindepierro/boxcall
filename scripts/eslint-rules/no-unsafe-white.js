...
export const noUnsafeWhiteRule = {
  meta: {
    type: "problem",
    docs: {
      description: "Disallow text-white on insufficiently dark backgrounds",
      recommended: false,
    },
    schema: [
      {
    ...
    messages: {
      unsafe:
        "Avoid `text-white` on light/unknown background. Use semantic text token or ensure approved dark bg class is present.",
    },
  },
  create(context) {
    const options = context.options?.[0] || {};
    const allowBg = new Set(
      options.allowBg || [
        "bg-jade-600",
        "bg-jade-700",
        "bg-jade-800",
