...
const output = execSync('grep -rln "logError(" --include="*.ts" --include="*.tsx" src/ 2>/dev/null | grep -v ".test." | grep -v "logger.ts"', { encoding: 'utf8' });
...
files.forEach(file => {
  try {
    ...
    if (content.includes('from') && (content.includes('/logger"') || content.includes("/logger'"))) {
      skipped++;
      return;
    }
