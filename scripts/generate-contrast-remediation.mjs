...
if(!fs.existsSync(reportPath)){
  console.error('contrast-report.json not found. Run: node scripts/contrast-inventory.mjs --json > contrast-report.json');
  process.exit(1);
}
...
for(const f of r.results){
  const score=(f.whiteOnLightPairs?3:0)+(f.midToneWhite.length?2:0)+(f.lightBgButtons.length?1:0);
  ...
  if(score>=4) high.push(line); else if(score>=2) medium.push(line); else low.push(line);
}
md += '### High\n'+(high.join('\n')||'None')+'\n\n';
md += '### Medium\n'+(medium.join('\n')||'None')+'\n\n';
md += '### Low\n'+(low.join('\n')||'None')+'\n\n';
