...
function log(msg) { console.log(`[perf-budgets] ${msg}`); }
function fail(msg) { console.error(`❌ [perf-budgets] ${msg}`); process.exitCode = 1; }
...
const initialTotal = initialJsFiles.reduce((sum, f) => sum + gzipSize(path.join(distAssetsDir, f)), 0);

log(`Largest non-pdf chunk gzip: ${largestNonPdf.size} bytes (${largestNonPdf.file})`);
log(`Initial JS (index/vendor) total gzip: ${initialTotal} bytes [${initialJsFiles.join(', ')}]`);

if (budgets.largestNonPdfChunkGzipTarget && largestNonPdf.size > budgets.largestNonPdfChunkGzipTarget) {
  fail(`Largest non-pdf chunk exceeded budget: ${largestNonPdf.size} > ${budgets.largestNonPdfChunkGzipTarget}`);
}
if (budgets.initialJsGzipTarget && initialTotal > budgets.initialJsGzipTarget) {
  fail(`Initial JS exceeded budget: ${initialTotal} > ${budgets.initialJsGzipTarget}`);
}

if (process.exitCode === 1) {
  console.error('\nBudget failure details saved to perf-budget-report.json');
  fs.writeFileSync(path.join(root, 'perf-budget-report.json'), JSON.stringify({ largestNonPdf, initialTotal, budgets }, null, 2));
} else {
