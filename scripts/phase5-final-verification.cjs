...
function countPatternInFiles(pattern, includePattern) {
  try {
    const result = execSync(`grep -r "${pattern}" src/ --include="${includePattern}" || true`, { encoding: 'utf8' });
...
function getMatches(pattern, includePattern, maxResults = 10) {
  try {
    const result = execSync(`grep -r "${pattern}" src/ --include="${includePattern}" | head -${maxResults} || true`, { encoding: 'utf8' });
...
console.log('\n3️⃣  HEX COLOR AUDIT:');
const hexMatches = getMatches('#[0-9a-fA-F]{6}', '*.{tsx,ts}').filter(match => 
...
if (fs.existsSync(tokensPath)) {
  console.log('   ✅ Central token system exists (/src/design-system/tokens.ts)');
} else {
...
const legacyColorsPath = path.join(__dirname, '..', 'src/components/design-system/Colors.tsx');
...
const interactionJadeCount = countPatternInFiles('bg-interaction-jade', '*.{tsx,ts}');
...
const totalNewTokenUsage = brandJadeCount + interactionJadeCount + surfaceJadeCount;
const remainingIssues = jadeMatches.length + navyMatches.length + (hexMatches.length > 2 ? hexMatches.length : 0);
...
if (remainingIssues === 0 && totalNewTokenUsage > 10) {
  ...
  console.log(`📝 ${remainingIssues} minor issues to address in future iterations`);
} else {
