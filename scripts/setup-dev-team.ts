...
if (!supabaseUrl || !supabaseServiceKey) {
  ...
  console.log('Required env vars: VITE_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY (or VITE_SUPABASE_ANON_KEY)');
  process.exit(1);
}
...
async function setupDevelopmentTeam() {
  ...
  try {
    ...
    if (!existingProfile) {
      ...
      console.log('✅ User profile exists:', existingProfile.full_name || 'Unnamed User');
    }
    ...
    if (!existingPlayers || existingPlayers.length === 0) {
      ...
      if (playersError) {
        console.log('⚠️  Sample players creation result:', playersError.message);
      } else {
