...
const newRawSurfaceEntries = currentRawSurfaceKeys.filter(
  (k) => !baselineSet.has(k)
);
const rawSurfaceCountGrowth =
  currentRawSurfaceKeys.length > (baseline.rawSurfaces?.count || 0);
if (newRawSurfaceEntries.length || rawSurfaceCountGrowth) {
  ...
  console.log(
    `Raw surface containers: OK (baseline=${baseline.rawSurfaces.count}, current=${currentRawSurfaceKeys.length})`
  );
}
