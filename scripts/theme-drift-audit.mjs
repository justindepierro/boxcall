...
const hexRegexes = hexPalette.map(
  (h) => new RegExp(h.replace("#", "\\#"), "gi")
);
...
const summary = {
  ...
  byCategory: Object.fromEntries(
    Object.entries(buckets).map(([k, list]) => [
      k,
      { files: list.length, matches: list.reduce((a, c) => a + c.count, 0) },
    ])
  ),
  topOffenders: results
    .slice(0, 25)
    .map((r) => ({ file: r.file, count: r.count, category: r.category })),
};
...
writeFileSync(
  "docs/style-inventory/theme-drift-audit.json",
