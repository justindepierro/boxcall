...
function findHexColorsInFile(filePath: string, project: Project): Replacement[] {
...
async function main() {
  ...
  const sourceFiles = project.getSourceFiles()
    .filter(sf => {
      ...
      return (path.endsWith('.tsx') || path.endsWith('.ts')) &&
             ...
             !path.includes('token-automation'); // Don't replace in automation scripts
    });

  console.log(`🔍 Scanning ${sourceFiles.length} files for exact hex matches...\n`);
  ...
  console.log(`📊 Found ${allReplacements.length} exact hex matches across ${fileReplacements.size} files\n`);
  ...
  console.log(`   tsx scripts/token-automation/01-replace-exact-hex-matches.ts --apply\n`);
  ...
  if (process.argv.includes('--apply')) {
    ...
    for (const [filePath, replacements] of fileReplacements) {
      ...
      uniqueReplacements.forEach((replacement, original) => {
        ...
        const regex = new RegExp(`(['"\`])${original.replace('#', '\\#')}\\1`, 'g');
