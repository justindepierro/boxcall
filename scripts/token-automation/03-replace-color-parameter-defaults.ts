...
async function main() {
  ...
  console.log(
    `🔍 Scanning ${sourceFiles.length} files for color parameter defaults...\n`
  );
  ...
  console.log(
    `📊 Found ${allReplacements.length} color defaults across ${fileReplacements.size} files\n`
  );
  ...
  fileReplacements.forEach((replacements, filePath) => {
    ...
    replacements.forEach((r) => {
      console.log(
        `   Line ${r.line}: ${r.functionName}(${r.parameterName} = ${r.original})`
      );
  ...
  if (process.argv.includes("--apply")) {
    ...
    for (const [filePath, replacements] of fileReplacements) {
      ...
      // Replace defaults
      replacements.forEach((r) => {
        const parameters = sourceFile.getDescendantsOfKind(
          SyntaxKind.Parameter
        );
        parameters.forEach((param) => {
          if (
            param.getName() === r.parameterName &&
            param.getStartLineNumber() === r.line
          ) {
            const initializer = param.getInitializer();
            if (initializer) {
