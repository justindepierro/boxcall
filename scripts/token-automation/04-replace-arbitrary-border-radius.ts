    ...
    while ((match = regex.exec(line)) !== null) {
      ...
      if (BORDER_RADIUS_MAPPINGS[`${pxValue}px`]) {
        ...
        const newValue = prefix
          ? tailwindClass.replace("rounded", `rounded${prefix}`)
          : tailwindClass;
...
function applyReplacements(
  ...
  replacements.forEach((r) => {
    content = content.replace(
      new RegExp(r.oldValue.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g"),
      r.newValue
    );
  });
...
async function main() {
  ...
  console.log(
    `🔍 Scanning ${sourceFiles.length} files for arbitrary border radius...\n`
  );
  ...
  console.log(
    `📊 Found ${totalReplacements} arbitrary border radius values across ${allReplacements.size} files\n\n`
  );
