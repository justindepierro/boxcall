...
function applyReplacements(
  ...
  sorted.forEach((r) => {
    const regex = new RegExp(r.oldValue.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi');
...
async function main() {
  ...
  console.log(`🔍 Scanning ${targetFiles.length} component/page files for hex colors...\n`);
  ...
  console.log(`📊 Found ${allReplacements.length} hex colors across ${fileMap.size} files\n\n`);
  ...
  if (!applyChanges) {
    ...
    console.log('   tsx scripts/token-automation/04-replace-component-hex-colors.ts --apply\n');
    return;
  }
