...
function applyReplacements(
  ...
  replacements.forEach((r) => {
    const regex = new RegExp(r.oldValue.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g');
...
async function main() {
  ...
  console.log(`🔍 Scanning ${targetFiles.length} component/page files for border radius patterns...\n`);
  ...
  console.log(`📊 Found ${allReplacements.length} border radius patterns across ${fileMap.size} files\n\n`);

  // Display by pattern
  const byPattern = allReplacements.reduce((acc, r) => {
  ...
  Object.entries(byPattern).forEach(([pattern, replacements]) => {
    console.log(`${pattern} → ${replacements[0].newValue} (${replacements.length} occurrences)`);
  });
  console.log('');
  ...
  if (!applyChanges) {
    ...
    console.log('   tsx scripts/token-automation/05-replace-border-radius.ts --apply\n');
    return;
  }
