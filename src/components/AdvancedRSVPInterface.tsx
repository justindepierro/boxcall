...
import { useAdvancedRSVP, useRSVPAnalytics } from '../hooks/useEnhancedCalendar';
import type { 
  AdvancedRSVP, 
  RSVPStatus, 
  RSVPCondition, 
...
export function AdvancedRSVPInterface({
  ...
  const { rsvp, loading, error, updateRSVP, sendReminder } = useAdvancedRSVP(eventId, userId);
    ...
    <div className="space-y-6">
      ...
      {userRole === 'coach' && (
        <div className="bg-gray-50 rounded-lg p-4">
          <h4 className="text-md font-medium text-gray-900 mb-2">Coach Controls</h4>
          <button
      ...
      {!canRespond && !rsvp && (
        <div className="text-center py-8 text-gray-500">
          {isDeadlinePassed ? 'RSVP deadline has passed' : 'No RSVP response yet'}
        </div>
      )}
    </div>
  );
...
function RSVPForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const rsvpData: Partial<AdvancedRSVP> = {
      ...formData,
      conditions: formData.response_type === 'conditional' ? conditions : undefined,
    ...
    <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-lg p-6 space-y-6">
      ...
      <div>
        ...
        <div className="space-y-2">
          {(['attending', 'not_attending', 'maybe', 'late', 'early_departure'] as RSVPStatus[]).map((status) => (
            <label key={status} className="flex items-center">
              <input
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, status: e.target.value as RSVPStatus }))}
        ...
        <div className="space-y-2">
            <label className="flex items-center">
              <input
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, response_type: e.target.value as AdvancedRSVP['response_type'] }))}
                className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300"
              />
              <span className="ml-2 text-gray-700">Simple Response</span>
            </label>          {allowConditional && (
            <label className="flex items-center">
              <input
                type="radio"
                name="response_type"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateCondition(index, { if_condition: e.target.value })}
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => updateCondition(index, { then_status: e.target.value as RSVPStatus })}
                      className="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm"
                    >
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setFormData(prev => ({ ...prev, transportation: e.target.value as AdvancedRSVP['transportation'] }))}
              className="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm"
            >
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) => setFormData(prev => ({ 
                  ...prev, 
                  attendee_names: e.target.value.split(',').map(name => name.trim()).filter(Boolean)
                }))}
                rows={2}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setEmergencyContact(prev => ({ ...prev, name: e.target.value } as EmergencyContact))}
                required
                className="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setEmergencyContact(prev => ({ ...prev, relationship: e.target.value } as EmergencyContact))}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setEmergencyContact(prev => ({ ...prev, phone: e.target.value } as EmergencyContact))}
                required
                className="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setEmergencyContact(prev => ({ ...prev, email: e.target.value } as EmergencyContact))}
                className="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm"
              />
            </div>
          </div>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-3">
          How confident are you about your response? ({formData.confidence_level}/5)
        </label>
        <input
          type="range"
          min="1"
          ...
          onChange={(e) => setFormData(prev => ({ ...prev, confidence_level: parseInt(e.target.value) as 1|2|3|4|5 }))}
...
function RSVPDisplay({ rsvp }: RSVPDisplayProps) {
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
      <h4 className="text-md font-medium text-gray-900 mb-4">Your RSVP Response</h4>
      ...
      <div className="space-y-3">
        ...
        {rsvp.transportation && (
          <div>
            ...
            <span className="text-sm text-gray-600 capitalize">{rsvp.transportation}</span>
          </div>
        )}
        ...
        {rsvp.confidence_level && (
          <div>
            ...
            <span className="text-sm text-gray-600">{rsvp.confidence_level}/5</span>
          </div>
        )}
...
function RSVPAnalyticsPanel({ eventId }: RSVPAnalyticsPanelProps) {
  const { analytics, loading, error, sendBulkReminders, exportData } = useRSVPAnalytics(eventId);
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-blue-50 rounded-lg p-4">
          <div className="text-2xl font-bold text-blue-900">{analytics.total_responded}</div>
          <div className="text-sm text-blue-700">Total Responses</div>
        </div>
        <div className="bg-green-50 rounded-lg p-4">
          <div className="text-2xl font-bold text-green-900">{analytics.status_breakdown.attending || 0}</div>
          <div className="text-sm text-green-700">Attending</div>
        </div>
        <div className="bg-red-50 rounded-lg p-4">
          <div className="text-2xl font-bold text-red-900">{analytics.status_breakdown.not_attending || 0}</div>
          <div className="text-sm text-red-700">Not Attending</div>
        </div>
        <div className="bg-yellow-50 rounded-lg p-4">
          <div className="text-2xl font-bold text-yellow-900">{analytics.response_rate.toFixed(1)}%</div>
      ...
      {/* Status Breakdown */}
      <div className="mb-6">
        <h5 className="text-md font-medium text-gray-900 mb-3">Response Breakdown</h5>
        <div className="space-y-2">
          {Object.entries(analytics.status_breakdown).map(([status, count]) => {
            const percentage = analytics.total_invited > 0 ? (count / analytics.total_invited) * 100 : 0;
            return (
      ...
      {/* Average Confidence */}
      <div className="mb-6">
        <h5 className="text-md font-medium text-gray-900 mb-2">Average Confidence</h5>
        <div className="flex items-center space-x-3">
          ...
          <span className="text-sm text-gray-600">{analytics.average_confidence.toFixed(1)}/5</span>
        </div>
      </div>
      ...
      {analytics.late_responders.length > 0 && (
        <div>
          <h5 className="text-md font-medium text-gray-900 mb-2">Need Reminders</h5>
