...
import { Calendar, AlertTriangle, TrendingUp, Brain, Target, CheckCircle } from 'lucide-react';
...
export const IntelligentCalendarDemo: React.FC<IntelligentCalendarDemoProps> = ({ 
  teamId = 'demo-team-1',
  className = ''
}) => {
  // ==========================================
  ...
  const [selectedFeature, setSelectedFeature] = useState<'conflicts' | 'scheduling' | 'analytics' | 'overview'>('overview');
  ...
  const renderFeatureNav = () => (
    <div className="flex flex-wrap gap-2 mb-6">
      {[
        ...
        { key: 'conflicts' as const, label: '⚠️ Conflicts', icon: AlertTriangle },
        { key: 'scheduling' as const, label: '📅 Smart Scheduling', icon: Calendar },
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-jade-50 to-emerald-50 rounded-xl p-6 border border-jade-200">
        ...
        <p className="text-jade-700 mb-4">
          Experience the next generation of smart sports scheduling with AI-powered conflict detection, 
          optimized timing suggestions, and predictive attendance analytics.
        </p>
        
        <div className="grid md:grid-cols-3 gap-4">
          <div className="bg-white rounded-lg p-4 border border-jade-200">
            <AlertTriangle className="w-8 h-8 text-amber-500 mb-2" />
            <h4 className="font-semibold text-slate-800 mb-1">Conflict Detection</h4>
          ...
          <div className="bg-white rounded-lg p-4 border border-jade-200">
            ...
            <h4 className="font-semibold text-slate-800 mb-1">Predictive Analytics</h4>
            <p className="text-sm text-slate-600">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <h4 className="font-semibold text-slate-800">🎯 Complete Analysis</h4>
          ...
          <p className="text-slate-600 text-sm mb-3">
            Run comprehensive conflict detection and optimization for your demo event
          </p>
          {isAnyLoading && (
            <div className="text-xs text-jade-600 font-medium">Processing...</div>
          )}
        </button>
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-lg border border-slate-200 p-6">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <div className="bg-slate-50 rounded-lg p-4 space-y-2">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{demoEvent.startTime.toLocaleDateString()}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-600">Time:</span>
                <span className="font-medium">
                  {demoEvent.startTime.toLocaleTimeString()} - {demoEvent.endTime.toLocaleTimeString()}
                </span>
              </div>
              <div className="flex justify-between">
          ...
          <div className="space-y-4">
            <h4 className="font-semibold text-slate-700">Conflict Types Checked</h4>
            <div className="space-y-2">
              {[
                { type: 'Team Conflicts', description: 'Other team events at same time' },
                { type: 'Coach Availability', description: 'Coach scheduling conflicts' },
                { type: 'Venue Conflicts', description: 'Field/facility double-booking' },
                { type: 'Academic Calendar', description: 'School events and holidays' },
                { type: 'Travel Considerations', description: 'Away game travel logistics' }
              ].map(({ type, description }) => (
                <div key={type} className="flex items-start gap-3 p-3 bg-slate-50 rounded-lg">
        ...
        {hasConflicts && (
          <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-lg">
            <h5 className="font-semibold text-red-800 mb-2">⚠️ Conflicts Detected</h5>
            <p className="text-red-700 text-sm">
              Demo conflicts would appear here with detailed information and suggested resolutions.
            </p>
          </div>
        )}
        ...
        {!hasConflicts && !isAnyLoading && (
          <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
            <h5 className="font-semibold text-green-800 mb-2">✅ No Conflicts Found</h5>
            <p className="text-green-700 text-sm">
              Your event is clear! No scheduling conflicts detected across all systems.
            </p>
          </div>
        )}
      </div>
  ...
  const renderSchedulingDemo = () => (
    <div className="space-y-6">
      <div className="bg-white rounded-lg border border-slate-200 p-6">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <h4 className="font-semibold text-slate-700">Optimization Parameters</h4>
            <div className="space-y-3">
              {[
                { param: 'Event Type', value: 'Practice Session' },
                { param: 'Duration', value: '2 hours' },
                { param: 'Preferred Days', value: 'Tuesday, Wednesday, Thursday' },
                { param: 'Preferred Times', value: '4:00 PM - 6:00 PM' },
                { param: 'Weather Sensitive', value: 'Yes' },
                { param: 'Attendance Priority', value: 'High' }
              ].map(({ param, value }) => (
                <div key={param} className="flex justify-between items-center p-2 bg-slate-50 rounded">
            ...
            >
              ...
              {isAnyLoading ? 'Optimizing Schedule...' : 'Generate Smart Suggestions'}
            </button>
          </div>
          
          <div className="space-y-4">
            <h4 className="font-semibold text-slate-700">AI Optimization Factors</h4>
            <div className="space-y-2">
              {[
                { factor: 'Historical Attendance', weight: '35%', status: 'optimal' },
                { factor: 'Weather Patterns', weight: '20%', status: 'good' },
                { factor: 'Team Preferences', weight: '20%', status: 'optimal' },
                { factor: 'Venue Availability', weight: '15%', status: 'good' },
                { factor: 'Academic Schedule', weight: '10%', status: 'optimal' }
              ].map(({ factor, weight, status }) => (
                <div key={factor} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                  <div className="flex-1">
                    <div className="font-medium text-slate-800 text-sm">{factor}</div>
                    <div className="text-xs text-slate-600">Weight: {weight}</div>
                  </div>
                  <div className={`px-2 py-1 rounded text-xs font-medium ${
                    status === 'optimal' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'
                  }`}>
                    {status}
                  </div>
                </div>
        ...
        {hasSuggestions && (
          <div className="mt-6 space-y-4">
            <h5 className="font-semibold text-slate-800">🎯 Optimization Results</h5>
              ...
              {suggestions.slice(0, 3).map((suggestion, index) => (
                <div key={index} className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  ...
                  <p className="text-blue-700 text-sm">
                    {suggestion.dateTime?.toLocaleString()} - Optimal scheduling based on data analysis
                  </p>
                </div>
              ))}
            </div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-6">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-6">
          <div className="bg-green-50 rounded-lg p-4 border border-green-200">
            ...
            <div className="text-xs text-green-600 mt-1">↑ 5.2% from last season</div>
          </div>
          
          <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
            <div className="text-2xl font-bold text-blue-800 mb-1">92.1%</div>
            <div className="text-sm text-blue-600 font-medium">Predicted Next Event</div>
            <div className="text-xs text-blue-600 mt-1">Based on 12 factors</div>
          </div>
          
          <div className="bg-amber-50 rounded-lg p-4 border border-amber-200">
            <div className="text-2xl font-bold text-amber-800 mb-1">4.3/5</div>
            <div className="text-sm text-amber-600 font-medium">Schedule Rating</div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <h4 className="font-semibold text-slate-700">Attendance Patterns</h4>
            <div className="space-y-2">
              {[
                ...
                <div key={day} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className={`w-2 h-2 rounded-full ${
                      ...
                      trend === 'down' ? 'bg-red-500' : 'bg-blue-500'
                    }`}></div>
                  </div>
                </div>
              ))}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {[
                { insight: 'Tuesday 5 PM sessions have highest attendance', impact: 'high' },
                { insight: 'Weather affects outdoor practice attendance by 15%', impact: 'medium' },
                { insight: 'Post-exam periods show 20% attendance drop', impact: 'high' },
                { insight: 'Weekend morning sessions underperform', impact: 'medium' }
              ].map(({ insight, impact }) => (
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-slate-600 mt-1">
              Phase 3: AI-powered scheduling with conflict detection, optimization, and analytics
            </p>
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          <div className={`px-2 py-1 rounded text-xs font-medium ${
            hasConflicts ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'
          }`}>
      ...
      <div className="bg-slate-50 rounded-lg p-4 text-center">
        <p className="text-sm text-slate-600">
          🚀 <strong>Phase 3 Complete!</strong> Intelligent features ready for production use.
          Next: Phase 4.1 Cross-Platform Integration
        </p>
      </div>
    </div>
  );
