...
export function SaveHistoryPanel() {
  ...
  const [filter, setFilter] = useState<"all" | "success" | "error" | "warning" | "conflict">("all");
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {filteredHistory.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-secondary">
            ...
            <p className="text-sm">No {filter !== "all" && filter} operations yet</p>
          </div>
        ) : (
          filteredHistory
            .slice()
            ...
            .map((entry) => (
              ...
              >
                {/* Status Icon */}
                <div className={`p-1.5 rounded ${getStatusColor(entry.status)}`}>
