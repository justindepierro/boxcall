...
export const FormationBuilderPanel: React.FC<FormationBuilderPanelProps> = ({
  ...
  console.log('🏗️ [FormationBuilderPanel] Component mounted/re-rendered with playbookId:', playbookId);
  ...
  console.log('🏗️ [FormationBuilderPanel] Current state:', {
    ...
    playbookId,
    formations: allFormations.map(f => ({ id: f.id, name: f.name, direction: f.direction }))
  });

  const loadData = useCallback(async () => {
    console.log('🔄 [FormationBuilderPanel] loadData() called with playbookId:', playbookId);
    setLoading(true);
    try {
      console.log('📞 [FormationBuilderPanel] Calling FormationService.getFormationsByPlaybook...');
      ...
      console.log('✅ [FormationBuilderPanel] Received data:', {
        ...
        formations: formations.map(f => ({ id: f.id, name: f.name, direction: f.direction, playbook_id: f.playbook_id }))
      });
      
      setAllFormations(formations);
      setAvailablePersonnel(personnel);
      
      console.log('✅ [FormationBuilderPanel] State updated with formations:', formations.length);
    } catch (error) {
  ...
  useEffect(() => {
    console.log('⚡ [FormationBuilderPanel] useEffect triggered with playbookId:', playbookId);
    if (playbookId) {
      console.log('✅ [FormationBuilderPanel] playbookId is valid, calling loadData()');
      loadData();
    } else {
      console.log('❌ [FormationBuilderPanel] playbookId is empty/undefined, skipping loadData()');
    }
  }, [playbookId, loadData]);
        ...
        {loading ? (
          ...
          <div className="p-spacing-md bg-surface-muted rounded border border-border-secondary text-center">
            <Typography variant="body-sm" className="text-text-muted">
              No formations found. Create formations by adding plays with formation names first.
            </Typography>
          </div>
        ) : (
          <div className="relative">
            ...
            >
              <option value="">Choose a formation to edit... ({allFormations.length} available)</option>
              {allFormations.map(formation => (
                <option key={formation.id} value={formation.id}>
                  {formation.name} {formation.direction !== 'base' && `(${formation.direction === 'left' ? 'Left' : formation.direction === 'right' ? 'Right' : 'Base'})`}
                </option>
              ))}
            </select>
        ...
        <>
          ...
          <div className="p-spacing-md bg-surface-secondary rounded-lg border border-border-primary">
            ...
            {availablePersonnel.length === 0 ? (
              <div className="p-spacing-md bg-surface-muted rounded border border-border-secondary text-center">
                <Typography variant="body-sm" className="text-text-muted">
                  No personnel configurations found. Create personnel packages first.
                </Typography>
              </div>
            ) : (
              <>
          ...
          <div className="p-spacing-md bg-surface-secondary rounded-lg border border-border-primary">
            ...
            <Typography variant="caption" className="text-text-secondary mb-spacing-sm">
              Comma-separated tags for filtering (e.g., "twins, compressed, stack")
            </Typography>
