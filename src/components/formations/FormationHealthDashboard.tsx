...
> = ({ playbookId, onFormationUpdated }) => {
  ...
  const loadData = async () => {
    try {
      ...
      const matchesData: typeof matches = {};
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border p-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="text-center">
            <div className={`text-3xl font-bold ${getHealthColor("paired")}`}>
              {stats.paired}
            </div>
          ...
          <div className="text-center">
            <div
              className={`text-3xl font-bold ${getHealthColor("standalone")}`}
            >
              {stats.standalone}
            </div>
          ...
          <div className="text-center">
            <div className={`text-3xl font-bold ${getHealthColor("unpaired")}`}>
              {stats.unpaired}
            </div>
      ...
      {unpairedFormations.length > 0 && (
        <div className="bg-white rounded-lg border p-6">
          ...
          <p className="text-sm text-secondary mb-4">
            ...
            Review suggestions below or mark as standalone if no opposite is
            needed.
          </p>
            ...
            {unpairedFormations.map((formation) => (
              <div key={formation.id} className="border rounded-lg p-4">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <button
                    onClick={() => handleMarkAsStandalone(formation.id)}
                    disabled={
                          ...
                          <div className="flex-1">
                            ...
                            <div className="text-xs text-secondary mt-1">
                              ...
                              {match.nameMatch === "exact" &&
                                " • Exact name match"}
                              {match.nameMatch === "similar" &&
                                " • Similar name"}
                            </div>
                          </div>
                          <button
                            onClick={() =>
                              handleLink(formation.id, match.formation.id)
                            }
                            disabled={
                              actionLoading ===
                ...
                {(!matches[formation.id] ||
                  matches[formation.id].length === 0) && (
                  <div className="text-sm text-muted italic">
                    No good matches found. Consider creating an opposite
                    formation or marking as standalone.
                  </div>
                )}
              </div>
            ))}
