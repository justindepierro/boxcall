...
export const MobileDrawer: React.FC<MobileDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 md:hidden" role="dialog" aria-modal="true" aria-label={title}>
      ...
      <div className={`absolute top-0 bottom-0 ${side === 'right' ? 'right-0' : 'left-0'} ${widthClass} surface-card shadow-xl border-l border-subtle flex flex-col animate-slide-in`}>        
        <div className="flex items-center justify-between px-4 py-3 border-b border-subtle">
          <h2 className="text-sm font-semibold text-slate-900">{title}</h2>
          <Button variant="ghost" size="xs" onClick={onClose} className="p-1 h-auto w-auto" aria-label="Close drawer">
            <X className="h-4 w-4" />
          </Button>
        </div>
