...
interface ActivationFlags { startedAt?: number; first_play?: boolean; first_practice?: boolean; first_script_export?: boolean; }
...
function markFlag(flag: keyof ActivationFlags, telemetryType: string, extra?: Record<string, unknown>) {
  const flags = loadFlags();
  if (flags[flag] === true) return; // already set
  const signupTs = flags.startedAt || Date.now();
  telemetry.enqueue({ type: telemetryType, data: { timeFromSignupMs: Date.now() - signupTs, ...extra } });
  ...
  saveFlags(flags);
  try { window.dispatchEvent(new CustomEvent('activation:flag_set', { detail: { id: flag } })); } catch { /* ignore */ }
}
...
export function markFirstScriptExport(filename?: string) {
  markFlag('first_script_export', 'activation:first_script_export', { filename });
}
