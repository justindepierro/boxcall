...
function usePersonnelConfigData(params: {
  configurations: ReturnType<typeof usePersonnelConfigurations>["data"];
  personnel?: string;
}) {
  const { configurations, personnel } = params;
  ...
  return { personnelConfig, availablePositions };
}

function useVariationTagInput(params: {
  tags: string[];
  onTagsChange: (tags: string[]) => void;
}) {
  const { tags, onTagsChange } = params;
...
const AdvancedOptionsHeader: React.FC<{
  isOpen: boolean;
  onToggle: () => void;
}> = ({ isOpen, onToggle }) => (
  <Button
...
const FormationDetailsSection: React.FC<{
  fieldValues: ReturnType<typeof usePlayFieldValues>;
  formationType: string;
  formationDir: string;
  backAlign: string;
  backLeftOfQb: boolean;
...
const PlayDetailsSection: React.FC<{
  fieldValues: ReturnType<typeof usePlayFieldValues>;
  playDir: string;
  protection: string;
  checkInto: string;
...
const TagsAndMetadataSection: React.FC<{
  availablePositions: string[];
  rosterPlayers: ReturnType<typeof useRosterData>["players"];
  tags: string[];
  newTag: string;
...
const AdvancedOptionsContent: React.FC<{
  ...
  availablePositions: string[];
  rosterPlayers: ReturnType<typeof useRosterData>["players"];
  tagInput: ReturnType<typeof useVariationTagInput>;
  formationType: string;
  formationDir: string;
  backAlign: string;
  backLeftOfQb: boolean;
  ...
  <div className="space-y-lg mt-lg">
    <FormationDetailsSection
      ...
      onFormationDirChange={onFormationDirChange}
      onBackAlignChange={onBackAlignChange}
      onBackLeftOfQbChange={onBackLeftOfQbChange}
    ...
    <TagsAndMetadataSection
      ...
      handleAddTag={tagInput.handleAddTag}
      handleRemoveTag={tagInput.handleRemoveTag}
      key_positions={key_positions}
      key_players={key_players}
      personnel={personnel}
      onKeyPositionsChange={onKeyPositionsChange}
      onKeyPlayersChange={onKeyPlayersChange}
    />

    <AdditionalInfoSection
      fieldValues={fieldValues}
      oneWordPlay={oneWordPlay}
      wristbandNumber={wristbandNumber}
      description={description}
      onOneWordPlayChange={onOneWordPlayChange}
      onWristbandNumberChange={onWristbandNumberChange}
...
export const AdvancedOptionsSection: React.FC<AdvancedOptionsSectionProps> = ({
  ...
  const { availablePositions } = usePersonnelConfigData({
    configurations,
    personnel,
  });

  const tagInput = useVariationTagInput({ tags, onTagsChange });

  return (
    <div className="border-t border-secondary pt-lg">
      <AdvancedOptionsHeader isOpen={isOpen} onToggle={onToggle} />
