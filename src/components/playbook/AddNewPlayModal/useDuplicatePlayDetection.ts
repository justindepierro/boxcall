...
import { buildDuplicateIndex, isDuplicate, type InboundPlay } from '../../../utils/playDataStandardization';
  ...
  return useMemo(() => {
    ...
    const matchingPlays = isDup
      ? existingPlays.filter(
          (p) =>
            p.play_name.toLowerCase().trim() === currentPlay.play_name?.toLowerCase().trim() &&
            p.formation.toLowerCase().trim() === currentPlay.formation?.toLowerCase().trim()
        )
      : [];
    
    // Compute duplicate key for reference
    const duplicateKey = currentPlay.play_name && currentPlay.formation
