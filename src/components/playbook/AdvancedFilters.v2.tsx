  ...
  ({ onFiltersChange, activeFilters }) => {
    ...
    const handleQuickFilter = (filterId: string) => {
      const existingFilter = activeFilters.find(
        (f) => f.field === "playType" && f.value === filterId
      );

      if (existingFilter) {
        // Remove filter
        onFiltersChange(
          activeFilters.filter((f) => f.id !== existingFilter.id)
        );
      } else {
        // Add filter
        const newFilter: ActiveFilter = {
          ...
          value: filterId,
          label:
            QUICK_FILTERS.find((qf) => qf.id === filterId)?.label || filterId,
        };
                ...
                {selectedCategory === category.id && (
                  <div className="grid grid-cols-2 gap-2 pl-3">
                    {category.options.map((option) => {
                      const isActive = isCategoryOptionActive(
                        category.id,
                        option.value
                      );
                      return (
                        <button
                          key={option.value}
                          onClick={() =>
                            handleCategorySelect(category.id, option.value)
                          }
