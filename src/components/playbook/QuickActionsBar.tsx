...
export const QuickActionsBar: React.FC<QuickActionsBarProps> = ({
  ...
  return (
    <div
      className={`fixed ${isMobile ? "bottom-20 right-4" : "bottom-6 right-6"} z-fixed ${className}`}
    >
        ...
        <div className={`mb-4 space-y-${isMobile ? "3" : "2"}`}>
          {actions.map((action) => (
            ...
            >
              ...
              {isMobile && (
                <span className="text-sm font-medium text-primary px-3 py-2 bg-primary rounded-lg shadow-md">
                  {action.label}
                </span>
              )}
