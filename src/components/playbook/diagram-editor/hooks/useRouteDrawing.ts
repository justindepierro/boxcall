// Type guard for PlayerSprite
function isPlayerSprite(sprite: unknown): sprite is PlayerSprite {
  return (
    sprite !== null &&
    typeof sprite === "object" &&
    "getPlayer" in sprite &&
    typeof (sprite as any).getPlayer === "function"
  );
}
/**
...
export function useRouteDrawing(options: UseRouteDrawingOptions) {
        ...
        const clickedPlayer = playersLayer
          ...
          .find((sprite: unknown) => {
            if (isPlayerSprite(sprite)) {
              const player = (sprite as PlayerSprite).getPlayer();
   ...
   */
  useEffect(() => {
    if (!isEnabled || !app || !app.app || !app.app.canvas) return;
