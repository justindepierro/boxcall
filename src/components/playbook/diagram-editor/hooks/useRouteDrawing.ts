...
import type { RouteType } from "../types/DiagramTypes";

export interface UseRouteDrawingOptions {
  app: DiagramPixiApp | null;
  routesLayer: RoutesLayer | null;
  playersLayer: PlayersLayer | null;
  isEnabled: boolean; // Only active when draw-route tool is selected
  routeType: RouteType; // Current route type from toolbar
}
