...
export interface UseWaypointEditingOptions {
  app: DiagramPixiApp | null;
  isEnabled: boolean; // Only active when edit-waypoint tool selected
}
...
export function useWaypointEditing({
  ...
  const routes = useDiagramStore((state) => state.routes);
  const updateRoute = useDiagramStore((state) => state.updateRoute);
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (e: PointerEvent) => {
      ...
      const updatedRoute = routesLayer.updateWaypointPosition(x, y, routes);
    ...
    const handlePointerUp = () => {
      ...
      if (editState) {
        ...
        const currentRoute = routes.find((r) => r.id === editState.routeId);
    ...
    const handleContextMenu = (e: MouseEvent) => {
      ...
      const waypoint = routesLayer.findWaypointAt(x, y, routes);
      if (waypoint) {
        const route = routes.find((r) => r.id === waypoint.routeId);
        if (!route) return;
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape" && isEditingRef.current) {
        ...
        const route = routes.find((r) => r.id === editingRouteIdRef.current);
        if (route) {
