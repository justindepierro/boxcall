...
export function detectWebGLCapabilities(): WebGLCapabilities {
  ...
  try {
    ...
    if (gl && result.supported) {
      ...
      if (debugInfo) {
        result.renderer = gl.getParameter(
          debugInfo.UNMASKED_RENDERER_WEBGL
        ) as string;
        result.vendor = gl.getParameter(
      ...
      result.maxViewportDims = gl.getParameter(gl.MAX_VIEWPORT_DIMS) as [
        number,
        number,
      ];
    ...
    result.error =
      error instanceof Error ? error.message : "Unknown WebGL detection error";
...
export function checkMinimumRequirements(): {
  ...
  if (capabilities.maxTextureSize && capabilities.maxTextureSize < 4096) {
    issues.push(
      `Maximum texture size (${capabilities.maxTextureSize}px) is below recommended (4096px)`
    );
  }
  ...
  if (capabilities.maxViewportDims) {
    ...
    if (maxWidth < 2048 || maxHeight < 2048) {
      issues.push(
        `Maximum viewport size (${maxWidth}×${maxHeight}) may limit canvas size`
      );
    }
  }
