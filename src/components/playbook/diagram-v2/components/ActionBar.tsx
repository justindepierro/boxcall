...
export const ActionBar: React.FC<{
  ...
  const player = useMemo(() => state.doc.players.find((p) => p.id === sel[0]), [state.doc.players, sel]);
  const selectedPlayers = useMemo(() => state.doc.players.filter((p) => sel.includes(p.id)), [state.doc.players, sel]);
  ...
  const allLocked = selectedPlayers.length > 0 && selectedPlayers.every((p) => !!p.locked);
      ...
      >
        <div className="bg-white/95 backdrop-blur rounded-md shadow-lg border border-slate-200 px-3 py-2">
          <div className="text-xs font-medium text-slate-700 mb-2">
            {multi ? `Selected: ${sel.length} players` : `Selected: ${player.id}`}
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <input
              ...
              onBlur={() => {
                ...
                dispatch({ type: "UPDATE_PLAYER", id: player.id, patch: { label } });
              }}
              disabled={multi}
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (multi) dispatch({ type: "UPDATE_PLAYERS_BULK", ids: sel, patch: { role } });
                else dispatch({ type: "UPDATE_PLAYER", id: player.id, patch: { role } });
              }}
              aria-label="Role"
            >
            ...
            <div className="grow min-w-[180px]">
              <input
                ...
                onBlur={() => {
                  ...
                  if (multi) dispatch({ type: "UPDATE_PLAYERS_BULK", ids: sel, patch: { assignment } });
                  else dispatch({ type: "UPDATE_PLAYER", id: player.id, patch: { assignment } });
                }}
            ...
            <div className="flex items-center gap-1">
              {colors.map((c) => (
                <Button
                  ...
                  title="Set color"
                  onClick={() => {
                    if (multi) dispatch({ type: "UPDATE_PLAYERS_BULK", ids: sel, patch: { color: c } });
                    else dispatch({ type: "UPDATE_PLAYER", id: player.id, patch: { color: c } });
                  }}
            ...
            <div className="flex items-center ml-auto">
              <Button
                ...
                onClick={() => {
                  ...
                  if (multi) dispatch({ type: "UPDATE_PLAYERS_BULK", ids: sel, patch: { locked: next } });
                  else dispatch({ type: "UPDATE_PLAYER", id: player.id, patch: { locked: next } });
                }}
              >
