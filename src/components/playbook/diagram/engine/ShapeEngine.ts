...
export class ShapeEngine {
  ...
  static fieldToCanvas(fieldX: number, fieldY: number, zoom: number = 1, panX: number = 0, panY: number = 0): Point {
  ...
  static canvasToField(canvasX: number, canvasY: number, zoom: number = 1, panX: number = 0, panY: number = 0): Point {
    const PIXELS_PER_YARD = 10 * zoom;
    return {
  ...
  static calculateSnapTargets(
    ...
    if (snapToGrid) {
      ...
      if (this.distance(point, { x: snappedX, y: point.y }) <= this.SNAP_THRESHOLD) {
        targets.push({
          type: 'grid',
      ...
      if (this.distance(point, { x: point.x, y: snappedY }) <= this.SNAP_THRESHOLD) {
        targets.push({
          type: 'grid',
  ...
  static getBestSnapTarget(point: Point, targets: SnapTarget[]): SnapTarget | null {
    ...
    return targets.reduce((best, current) => {
      ...
      if (currentDistance < bestDistance) {
        return current;
      } else if (currentDistance === bestDistance && current.strength > best.strength) {
        return current;
      }
      return best;
    });
  ...
  static getRotationHandle(center: Point, rotation: number = 0): Point {
    ...
    return {
      x: center.x + Math.cos(angle - Math.PI / 2) * this.ROTATION_HANDLE_DISTANCE,
      y: center.y + Math.sin(angle - Math.PI / 2) * this.ROTATION_HANDLE_DISTANCE
    };
  }
  ...
  static getHandleAtPoint(point: Point, handles: ShapeHandle[], threshold: number = 8): ShapeHandle | null {
      ...
      case 'corner': {
        ...
        switch (cornerIndex) {
          case 0: // top-left
            ...
            bounds.width = originalBounds.x + originalBounds.width - dragPoint.x;
            bounds.height = originalBounds.y + originalBounds.height - dragPoint.y;
            break;
          case 1: // top-right
            bounds.y = dragPoint.y;
  ...
  static calculateRouteBounds(segments: Array<{ start: Point; end: Point }>): Bounds {
  ...
  static isNearPoint(point: Point, target: Point, threshold: number = this.SNAP_THRESHOLD): boolean {
