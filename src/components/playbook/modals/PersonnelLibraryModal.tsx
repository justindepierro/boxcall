...
function filterPersonnel(
  ...
  return personnel.filter((p) => p.name.toLowerCase().includes(normalized));
}

function computeStats(
  personnel: PersonnelConfiguration[],
  filteredPersonnel: PersonnelConfiguration[]
): PersonnelLibraryStats {
  return {
...
const PersonnelLibraryBackdrop: React.FC<{ onClose: () => void }> = ({
  onClose,
}) => (
  <div
...
type PersonnelLibraryHeaderProps = {
  onClose: () => void;
  onCreateNew: () => void;
...
const PersonnelLibraryHeader: React.FC<PersonnelLibraryHeaderProps> = ({
  onClose,
  onCreateNew,
  onImportFromPlays,
  onUpdateUsage,
...
type PersonnelLibrarySearchProps = {
  searchQuery: string;
  onChangeSearchQuery: (value: string) => void;
};

const PersonnelLibrarySearch: React.FC<PersonnelLibrarySearchProps> = ({
  searchQuery,
  onChangeSearchQuery,
}) => (
...
type PersonnelCardProps = {
  ...
  onEditBadge: (config: PersonnelConfiguration) => void;
};
...
type PersonnelLibraryContentProps = {
  loading: boolean;
  filteredPersonnel: PersonnelConfiguration[];
  onEditBadge: (config: PersonnelConfiguration) => void;
};

const PersonnelLibraryContent: React.FC<PersonnelLibraryContentProps> = ({
  loading,
  filteredPersonnel,
  onEditBadge,
}) => {
...
const PersonnelLibraryFooterStats: React.FC<{
  stats: PersonnelLibraryStats;
}> = ({ stats }) => (
...
export const PersonnelLibraryModal: React.FC<PersonnelLibraryModalProps> = ({
  ...
  const filteredPersonnel = filterPersonnel(personnel, searchQuery);
  const stats = computeStats(personnel, filteredPersonnel);

  return (
    <>
      <PersonnelLibraryBackdrop onClose={onClose} />
      ...
      <div className="fixed top-16 left-1/2 -translate-x-1/2 w-[95vw] md:w-[85vw] lg:w-[75vw] xl:w-[65vw] h-[calc(100vh-5rem)] bg-white dark:bg-navy-900 z-50 shadow-2xl rounded-lg overflow-hidden flex flex-col animate-fade-in">
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-white dark:bg-navy-900">
          <PersonnelLibraryContent
            loading={loading}
            filteredPersonnel={filteredPersonnel}
            onEditBadge={setEditingPersonnel}
          />
        </div>
        <PersonnelLibraryFooterStats stats={stats} />
      </div>
