      ...
      >
        <div className="p-6">
          <PracticePlannerHeader
            ...
            onClose={onClose}
            onTimeAllocationModeToggle={() => practiceState.setTimeAllocationMode(!practiceState.timeAllocationMode)}
            onScaffoldModeToggle={() => practiceState.setScaffoldMode(!practiceState.scaffoldMode)}
            onPracticeBlocksChange={practiceState.setPracticeBlocks}
            onOriginalBlocksChange={practiceState.setOriginalBlocksBeforeScaffold}
            ...
            <>
              ...
              {practiceState.showAddBlock && (
                <AddBlockModal
                  ...
                  onAddBlock={() => {/* TODO: Implement */}}
                  onCancel={() => practiceState.setShowAddBlock(false)}
                  onRoleSwitch={() => practiceState.setUserRole(practiceState.userRole === 'head_coach' ? 'position_coach' : 'head_coach')}
                />
              )}
            </>
          )}
          ...
          {/* Action Footer */}
          <ActionFooter
            isOvertime={practiceState.totalDuration > practiceState.scheduledDuration}
            totalDuration={practiceState.totalDuration}
            scheduledDuration={practiceState.scheduledDuration}
            onClose={onClose}
            onSaveWithOvertime={() => {/* TODO: Implement save */}}
        ...
        <ScriptSelectorModal
          ...
          selectedGroupId={practiceState.selectedGroupForScript?.groupId || null}
          onClose={() => practiceState.setShowScriptSelector(false)}
          onScriptSelect={() => {/* TODO: Implement */}}
        />
      </div>
    </div>
  );
