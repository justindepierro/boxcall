...
export const TeamTrophyCase: React.FC<TeamTrophyCaseProps> = () => {
  ...
  const mockTeamAchievements = {
    teamGoals: [
      { id: "1", name: "Undefeated Season", icon: "🏆", progress: 8, target: 10, completed: false },
      { id: "2", name: "League Championship", icon: "👑", progress: 1, target: 1, completed: true },
      { id: "3", name: "Team GPA 3.5+", icon: "📚", progress: 1, target: 1, completed: true },
    ],
    teamMedals: [
      { id: "1", name: "Perfect Practice Week", icon: "⭐", earned: true, date: "2024-08-10" },
      { id: "2", name: "Zero Penalties Game", icon: "🚫", earned: true, date: "2024-08-05" },
      { id: "3", name: "Community Service Champions", icon: "🤝", earned: false, progress: 75 },
    ],
    helmetStickers: {
      total: 47,
      thisWeek: 8,
    ...
    <Card className="p-6 bg-gradient-to-br from-navy-50 to-navy-100 dark:from-navy-900/20 dark:to-navy-800/20 border-navy-200 dark:border-navy-800">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-right">
          ...
          <Typography variant="body-lg" className="font-bold text-navy-600 dark:text-navy-400">
      ...
      <div className="mb-6">
        ...
        <div className="space-y-3">
          {mockTeamAchievements.teamGoals.map((goal) => (
            <div key={goal.id} className="p-3 bg-white dark:bg-gray-800 rounded-md">
      ...
      <div className="mb-6">
        ...
        <div className="space-y-2">
          {mockTeamAchievements.helmetStickers.categories.map((category, index) => (
            <div key={index} className="flex items-center justify-between p-2 bg-white dark:bg-gray-800 rounded-md">
              <div className="flex items-center space-x-2">
              ...
              <Typography variant="body-sm" className="font-bold text-jade-600 dark:text-jade-400">
                {category.count}
              </Typography>
            </div>
          ))}
        ...
        <div className="space-y-2">
          {mockTeamAchievements.teamMedals.map((medal) => (
            <div key={medal.id} className="flex items-center space-x-3 p-2 bg-white dark:bg-gray-800 rounded-md">
              <div className={`text-lg ${medal.earned ? '' : 'grayscale opacity-50'}`}>
                {medal.icon}
              </div>
