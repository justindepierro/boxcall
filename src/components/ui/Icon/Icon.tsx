...
import {
  ...
  ArrowLeft, ArrowRight, ArrowUp, ArrowDown, Home, Settings,
  Sidebar, Grid, List, MoreHorizontal, MoreVertical,
  ...
  Check, AlertCircle, Info, Archive, RotateCcw, RotateCw, Undo, Redo, RefreshCw, Power,
  
  // Communication & Data
  MessageSquare, Phone, Mail, FileText, Folder, FolderOpen,
  Search, Filter, SortAsc, SortDesc, Book, MessageCircle,
  Send, Reply, Forward, AtSign, Hash, Link, Paperclip,
  
  // Sports Specific
  Target, Flag, Award, Trophy, Medal, Flame, Crown, Star,
  TrendingUp, TrendingDown, BarChart3, PieChart, Activity,
  Zap, Shield, Lock, Unlock, Eye, EyeOff, Crosshair,
  ...
  // Food & Nutrition
  Coffee, Pizza, Apple, Utensils, ChefHat, Cookie,
  ...
  // Technology
  Smartphone, Laptop, Monitor, Tablet, Mouse, Keyboard,
  Wifi as WifiIcon, Bluetooth, Battery, BatteryLow, Cpu, HardDrive,
  
  // Shopping & Commerce
  ShoppingCart, ShoppingBag, CreditCard, DollarSign,
  Euro, PoundSterling, Receipt, Tag, Gift, Store,
  
  // Time & Calendar Extended
  AlarmClock, Hourglass, Watch, CalendarCheck,
  ...
  ThumbsUp, ThumbsDown, Share, Share2, Bookmark, BookmarkPlus,
  MessageSquare as Comment, Bell, BellOff, BellRing,
  
  // Games & Entertainment
  Gamepad2 as Gamepad, Dice1, Dice2, Dice3, Dice4, Dice5, Dice6,
  ...
  // Security & Safety
  Key, ShieldCheck, ShieldAlert, ShieldX, Fingerprint, Scan, QrCode,
  ...
  Info as InfoIcon, HelpCircle, MinusCircle, PlusCircle,
  
  // Movement & Direction Extended
  MoveUp, MoveDown, MoveLeft, MoveRight, Move, ArrowUpDown,
  ArrowLeftRight, ChevronsUp, ChevronsDown, ChevronsLeft, ChevronsRight,
  
  // PDF & Export
  FileDown, Printer, Share as ShareIcon,
...
export class SmartIconSystem {
   ...
   */
  static getIconSuggestions(content: string, maxSuggestions: number = 3): IconName[] {
    if (!content || typeof content !== 'string') {
  ...
  static getContextualIcon(
    content: string, 
    context: 'feed' | 'calendar' | 'achievement' | 'message' | 'team' | 'general' = 'general',
    ...
    if (context === 'achievement' && !['trophy', 'medal', 'award', 'crown', 'star'].includes(smartIcon)) {
      const achievementIcons = this.contentPatterns.achievement || ['trophy'];
      return achievementIcons[0];
    }
    
    if (context === 'calendar' && !['calendar', 'clock', 'timer'].includes(smartIcon)) {
      return 'calendar';
    }
