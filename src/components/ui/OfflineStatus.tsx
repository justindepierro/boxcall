...
export const NetworkStatusIndicator: React.FC<{
  ...
  if (compact) {
    return (
      <div className={`flex items-center space-x-1 ${className}`}>
        {isOnline ? (
          <Wifi className={`h-4 w-4 ${isSlowConnection ? 'text-yellow-500' : 'text-green-500'}`} />
        ) : (
  ...
  return (
    <div className={`flex items-center space-x-2 p-2 rounded-lg ${
      isOnline 
        ? (isSlowConnection ? 'bg-yellow-50 border-yellow-200' : 'bg-green-50 border-green-200')
      ...
      <div className="flex flex-col">
        <Typography variant="body-sm" className={
          isOnline 
            ? (isSlowConnection ? 'text-yellow-800' : 'text-green-800')
            : 'text-red-800'
        }>
        ...
        {isOnline && effectiveType && (
          <Typography variant="caption" className="text-gray-500">
            {effectiveType.toUpperCase()} • {isSlowConnection ? 'Slow connection' : 'Fast connection'}
          </Typography>
        )}
        {!isOnline && (
...
export const SyncStatusIndicator: React.FC<{
  ...
  const { pending, failed, isRefreshing, refresh, isOnline, hasPendingChanges } = useSyncStatus();

  if (!hasPendingChanges && !failed) {
    return showDetails ? (
      <div className={`flex items-center space-x-2 p-2 bg-green-50 border border-green-200 rounded-lg ${className}`}>
  ...
  return (
    <div className={`flex items-center justify-between p-2 bg-blue-50 border border-blue-200 rounded-lg ${className}`}>
...
export const DataFreshnessIndicator: React.FC<{
  ...
  const { isFresh, isStale, isVeryStale, lastSync } = useDataFreshness(type, id);
  ...
  return (
    <div className={`flex items-center space-x-1 ${className}`}>
      <Clock className={`h-3 w-3 ${
        isVeryStale ? 'text-red-500' : isStale ? 'text-yellow-500' : 'text-gray-500'
      }`} />
      {showAge && lastSync && (
        <Typography variant="caption" className={
          isVeryStale ? 'text-red-600' : isStale ? 'text-yellow-600' : 'text-gray-500'
        }>
          {lastSync}
        </Typography>
      )}
...
export const OfflineStatusBar: React.FC<{
  ...
  return (
    <div className={`flex items-center space-x-3 ${className}`}>
      {showNetworkStatus && (
        <NetworkStatusIndicator compact showLabel />
      )}
      {showSyncStatus && (
        <SyncStatusIndicator />
      )}
...
export const OfflineAwareContainer: React.FC<{
  ...
  className?: string;
}> = ({ children, showBanner = true, showStatusBar = false, className = '' }) => {
  return (
