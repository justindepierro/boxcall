...
export const analyticsConfig = {
  ...
  sentry: {
    ...
    enabled: Boolean(import.meta.env.VITE_SENTRY_DSN) || import.meta.env.VITE_ENABLE_SENTRY === 'true',
  ...
  general: {
    ...
    enableDebugMode: import.meta.env.VITE_ANALYTICS_DEBUG === 'true' || import.meta.env.DEV,
...
export function validateAnalyticsConfig(): {
  ...
  if (analyticsConfig.googleAnalytics.enabled) {
    if (!analyticsConfig.googleAnalytics.measurementId.startsWith('G-')) {
      errors.push('Invalid Google Analytics Measurement ID format (should start with G-)');
    }
  }
  ...
  if (analyticsConfig.posthog.enabled) {
    if (!analyticsConfig.posthog.apiHost.startsWith('http')) {
      errors.push('Invalid PostHog API host (should be a valid HTTP/HTTPS URL)');
    }
  }
  ...
  if (analyticsConfig.custom.enabled) {
    if (!analyticsConfig.custom.endpoint.startsWith('http')) {
      errors.push('Invalid custom analytics endpoint (should be a valid HTTP/HTTPS URL)');
    }
...
export function logAnalyticsConfig() {
  ...
  if (analyticsConfig.general.enableDebugMode) {
    ...
    console.log('Google Analytics:', {
      ...
      measurementId: analyticsConfig.googleAnalytics.measurementId ? '***configured***' : 'not set',
