...
export function canEditEvent(user: UserLike, event: CalendarEvent | null | undefined): boolean {
...
export function canDeleteEvent(user: UserLike, event: CalendarEvent | null | undefined): boolean {
  if (!user || !event) return false;
  if (user.role === 'admin') return true;
...
export function canEditRecurringInstance(user: UserLike, event: CalendarEvent | null | undefined): boolean {
