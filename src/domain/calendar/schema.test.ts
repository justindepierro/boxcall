...
import { parseCalendarEvent, parseCalendarEvents, parseEventRSVPs, parseCalendarComments, parseCalendarCommentCreate } from './schema';
...
describe('calendar schema parsing', () => {
  ...
  it('parses comments', () => {
    const comments = parseCalendarComments([
      { id: 'c1', event_id: 'e1', user_id: 'u1', body: 'Go team', created_at: '2025-01-01', updated_at: '2025-01-01' }
    ]);
    expect(comments[0].body).toBe('Go team');
  });

  it('rejects invalid comment create payload', () => {
    expect(() => parseCalendarCommentCreate({ event_id: 'e1', body: '' })).toThrow();
  });

  it('allows tags array (edge case empty)', () => {
    const ev = parseCalendarEvent({ id: 't1', title: 'Film', start: new Date().toISOString(), type: 'film', tags: [] });
  ...
  it('rejects invalid date format (edge)', () => {
    ...
    expect(() => parseCalendarEvent({ id: 'bad-date', title: 'Bad', start: 12345, type: 'practice' })).toThrow();
  });
});
