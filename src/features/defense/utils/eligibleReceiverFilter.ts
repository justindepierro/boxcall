...
export function getEligibleReceivers(players: Player[]): Player[] {
  return players.filter((player) => {
    ...
    if (
      INELIGIBLE_POSITIONS.includes(
        player.jerseyNumber as (typeof INELIGIBLE_POSITIONS)[number]
      )
    ) {
      return false;
    }
 ...
 */
export function countReceiversRight(
  players: Player[],
  centerX: number
): number {
  const eligible = getEligibleReceivers(players);
 ...
 */
export function countWideReceiversLeft(
  players: Player[],
  centerX: number
): number {
 ...
 */
export function countWideReceiversRight(
  players: Player[],
  centerX: number
): number {
  const wideReceivers = getWideReceivers(players);
