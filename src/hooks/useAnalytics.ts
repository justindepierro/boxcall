...
export function useAnalytics() {
  ...
  const trackTiming = useCallback((name: string, startTime: number, properties?: Record<string, any>) => {
...
export function useErrorTracking() {
  ...
  const captureApiError = useCallback((error: Error, endpoint: string, method: string, statusCode?: number) => {
    errorTracking.captureApiError(error, endpoint, method, statusCode);
  }, []);

  const captureUserAction = useCallback((error: Error, action: string, component?: string) => {
...
export function usePerformanceMonitoring() {
  ...
  const trackApiCall = useCallback((endpoint: string, method: string, duration: number, statusCode: number) => {
    ...
    if (duration > 2000) { // Flag slow API calls
  ...
  const measureComponentRender = useCallback(<T extends React.ComponentType<any>>(
    Component: T,
...
export function useUserTracking() {
  ...
  const trackUserEvent = useCallback((eventName: string, properties?: Record<string, any>) => {
...
export function useFeatureTracking() {
  ...
  const trackFeatureFlag = useCallback((flagName: string, variant: string, properties?: Record<string, any>) => {
  ...
  const trackExperiment = useCallback((experimentName: string, variant: string, properties?: Record<string, any>) => {
