...
export function useCDN() {
  ...
  const preloadAsset = useCallback(
    (src: string, type?: "image" | "script" | "style" | "font") => {
  ...
  const getResponsiveSrcSet = useCallback(
    (src: string, sizes: number[], options?: any) => {
...
export function useRouteAssets(routeName?: string) {
  ...
  const currentRoute =
    routeName || location.pathname.split("/")[1] || "dashboard";
...
export function useResponsiveImage(
  ...
  useEffect(() => {
    const updateSize = () => {
      ...
      const appropriateSize =
        sizes.find((size) => width <= size) || sizes[sizes.length - 1];
  ...
  return {
    currentSize,
    optimizedUrl: currentSize
      ? getOptimizedUrl(src, { width: currentSize })
      : src,
