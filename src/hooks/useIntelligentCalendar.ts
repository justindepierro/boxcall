...
} from "../services/phase3/AttendanceAnalyticsService";
import {
  ConflictDetectionService,
  type ConflictDetectionRequest,
  type ConflictDetectionResult,
} from "../services/phase3/ConflictDetectionService";
import {
  IntelligentCalendarService,
  type IntelligentCalendarRequest,
  type IntelligentCalendarResponse,
} from "../services/phase3/IntelligentCalendarService";
import {
 ...
 */
export const useIntelligentCalendar = (
  options: IntelligentCalendarHookOptions
) => {
  // ==========================================
  // State Management
  // ==========================================
    ...
    async (
      ...
      try {
        const constraints: SchedulingConstraints = {
          teamId: options.teamId,
          preferredDays: schedulingOptions.preferredDays || [
            "tuesday",
            "wednesday",
            "thursday",
          ],
          preferredTimeSlots:
            schedulingOptions.preferredTimes?.map((hour) => ({
              day: "tuesday",
              startHour: hour,
              endHour: hour + 2,
    ...
    async (eventId: string): Promise<AttendancePrediction | null> => {
      ...
      try {
        ...
        setState((prev) => ({
          ...prev,
          predictions: [
            ...prev.predictions.filter((p) => p.eventId !== eventId),
            prediction,
          ],
          isLoading: false,
        }));
   ...
   */
  const processIntelligentRequest = useCallback(
    async (
   ...
   */
  const checkEventAndSuggest = useCallback(
    async (
      proposedEvent: {
        startTime: Date;
        endTime: Date;
      ...
      if (conflicts?.hasConflicts) {
        await generateSuggestions({
          ...
          duration:
            (proposedEvent.endTime.getTime() -
              proposedEvent.startTime.getTime()) /
            (1000 * 60),
          ...options,
        });
      }
  ...
  return {
    ...
    isAnyLoading:
      state.isLoading ||
      state.isDetectingConflicts ||
      state.isGeneratingSuggestions ||
      state.isAnalyzingAttendance ||
