...
export class ColorGenerationService {
  ...
  static generateTeamPalette(teamColors: TeamColors): ColorPalette {
    ...
    const secondary = teamColors.secondary
      ? this.parseColor(teamColors.secondary)
      : this.generateComplementary(primary);
    const accent = teamColors.accent
      ? this.parseColor(teamColors.accent)
      : this.generateTriadic(primary)[0];
  ...
  private static generateComplementary(color: {
    r: number;
    g: number;
    b: number;
  ...
  private static generateTriadic(color: {
    r: number;
    g: number;
    b: number;
  }): [
  ...
  private static rgbToHsl(color: { r: number; g: number; b: number }): {
    h: number;
    s: number;
    l: number;
  } {
  ...
  private static hslToRgb(hsl: { h: number; s: number; l: number }): {
    r: number;
    g: number;
    b: number;
  } {
