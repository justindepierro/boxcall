...
export const DiagnosticsPage: React.FC = () => {
  const [vitals, setVitals] = useState<VitalRecord[]>(() => window.__WEB_VITALS__ || []);
  useEffect(() => {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto font-sans">
      ...
      <section className="mb-8">
        ...
        <pre className="bg-surface-secondary p-3 rounded text-sm overflow-auto">{JSON.stringify({ buildTime: window.__BUILD_META__?.buildTime || 'N/A', mode: window.__BUILD_META__?.mode || import.meta.env.MODE }, null, 2)}</pre>
      </section>
      <section>
        <h2 className="text-xl font-medium mb-2">Web Vitals</h2>
        {vitals.length === 0 && <p className="text-text-secondary text-sm">Waiting for metrics...</p>}
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {vitals.map(v => (
              <tr key={v.name} className="border-b border-border-subtle last:border-none">
                ...
                <td className="py-1 pr-2 capitalize">{v.rating.replace('-', ' ')}</td>
                <td className="py-1 pr-2 tabular-nums">{(v.value - v.threshold).toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
