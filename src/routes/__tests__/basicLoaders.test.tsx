...
import { createMemoryRouter, RouterProvider, type LoaderFunction } from "react-router-dom";
...
type Member = { role: "coach" | "admin" | "player"; status: "active" | "invited" | "suspended" };
const hoisted = vi.hoisted(() => ({ userId: "u1" as string | null, role: "coach" as unknown, member: { role: "coach", status: "active" as const } as Member }));

vi.mock("../../lib/supabase", () => ({
  supabase: {
    auth: { getUser: async () => ({ data: { user: hoisted.userId ? { id: hoisted.userId } : null } }) },
    from: (table: string) => ({
      select: () => {
        const chain = {
          eq: () => chain,
...
function renderWithRouter(path: string, route: TestRoute) {
  const router = createMemoryRouter(
    [
      { path: route.path, loader: route.loader, element: <OK label={route.label} /> },
...
describe("basic loaders", () => {
  ...
  it("requireAuthenticatedLoader allows authed user", async () => {
    renderWithRouter("/dashboard", { path: "/dashboard", loader: requireAuthenticatedLoader, label: "OK_AUTH" });
  ...
  it("requireAuthenticatedLoader redirects unauthenticated to login", async () => {
    hoisted.userId = null;
    renderWithRouter("/dashboard", { path: "/dashboard", loader: requireAuthenticatedLoader, label: "OK_AUTH" });
    await waitFor(async () => expect(await screen.findByText("LOGIN")).toBeInTheDocument());
  });

  it("requireTeamMemberLoader allows team member", async () => {
    renderWithRouter("/team/t1/bulletin", { path: "/team/:teamId/bulletin", loader: requireTeamMemberLoader, label: "OK_TEAM" });
