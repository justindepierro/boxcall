...
export async function authorize(input: AuthorizeInput): Promise<AuthorizeResult> {
  ...
  const needsTeam = Boolean(teamFeature || allowedTeamRoles?.length || requiredTiers?.length || requiredPermissions?.length);
  if (needsTeam) {
    ...
    if (!isSuperAdmin) {
      ...
      if (membership.status !== "active") return { allowed: false, reason: "inactive_member" };
  ...
  let subscription: Awaited<ReturnType<typeof fetchTeamSubscription>> | undefined;
  ...
  if ((requiredPermissions && requiredPermissions.length > 0) || teamFeature) {
    ...
    const teamRole: PermissionTeamRole | undefined = (membership?.role as unknown as PermissionTeamRole) || undefined;
    const subscriptionTier: PermissionSubscriptionTier = (subscription?.subscription_tier as PermissionSubscriptionTier) || "free";
    if (teamFeature) {
      const ok = canAccessTeamFeature(appUserType, teamRole, subscriptionTier, teamFeature);
      if (!ok) return { allowed: false, reason: "permission_denied", membership, subscription };
    }
