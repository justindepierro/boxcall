...
interface ContrastResult { theme: string; textVar: string; surfaceVar: string; contrast: number; pass: boolean; }
...
const textTokens = [
  'textPrimary','textSecondary','textMuted','textInverse','textBrand','success','warning','error'
];
const surfaceTokens = [
  'bgPrimary','bgSecondary','bgMuted','surfaceInverse','surfaceInverseAlt'
];
...
for (const theme of themeRegistry.themes) {
  ...
  const pass = ratio >= 4.5 || (['textSecondary','textMuted','textBrand','success','warning','error'].includes(txt) && ratio >= 3.0);
      results.push({ theme: theme.id, textVar: txt, surfaceVar: surf, contrast: +ratio.toFixed(2), pass });
    }
  }
}
