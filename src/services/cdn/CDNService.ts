...
export class CDNService {
  ...
  getWebPFallback(src: string, options?: ImageOptimizationOptions) {
    const webpUrl = this.getOptimizedImageUrl(src, {
      ...options,
      format: "webp",
    });
    ...
    return {
      ...
      cacheHitRate:
        this.imageCache.size > 0
          ? this.loadedAssets.size / this.imageCache.size
          : 0,
    };
  }
}
...
export class AssetLoader {
  ...
  static async preloadRouteAssets(routeName: string): Promise<void> {
    ...
    await Promise.all([
      ...
      ...routeAssets.scripts.critical.map((src) => this.loadScript(src)),
      // Preload critical styles
      ...routeAssets.styles.critical.map((href) => this.loadStylesheet(href)),
    ]);

    // Prefetch non-critical assets
    routeAssets.images.prefetch.forEach((src) => cdnService.prefetchAsset(src));
  }
}
