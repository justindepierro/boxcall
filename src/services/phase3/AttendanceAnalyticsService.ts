...
export class AttendanceAnalyticsService {
  ...
  static async calculateAttendanceTrends(
    ...
    for (const [dateKey, periodEvents] of groupedEvents) {
      ...
      trends.push({
        ...
        averageAttendance:
          periodEvents.length > 0 ? presentCount / periodEvents.length : 0,
  ...
  static async generatePlayerAnalytics(
    ...
    for (const member of teamMembers || []) {
      ...
      const absentCount = playerRecords.filter(
        (r) => r.status === "absent"
      ).length;
      ...
      analytics.push({
        ...
        lastEvent:
          playerRecords.length > 0
            ? new Date(
                Math.max(
                  ...playerRecords.map((r) => new Date(r.createdAt).getTime())
                )
              )
            : undefined,
        consistencyScore,
  ...
  static async analyzeEventTypes(
    ...
    for (const eventType of eventTypes) {
      ...
      const averageAttendance =
        typeEvents.length > 0 ? presentCount / typeEvents.length : 0;
    ...
    return [
      {
        ...
        actionItems: [
          ...
          "Survey team for preferred times",
        ],
      },
    ];
  }
