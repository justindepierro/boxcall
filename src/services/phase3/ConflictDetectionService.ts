...
export class ConflictDetectionService {
  ...
  static async detectConflicts(
    ...
    try {
      ...
      const [
        ...
        request.checkAcademicCalendar
          ? this.detectAcademicConflicts(request)
          : [],
        ...
        for (const event of teamEvents) {
          ...
          if (timeOverlap.hasOverlap) {
            conflicts.push({
              ...
              type: "team",
              severity:
                timeOverlap.overlapMinutes > 60 ? "critical" : "warning",
   ...
   */
  static async detectCoachConflicts(
    request: ConflictDetectionRequest
  ): Promise<Conflict[]> {
    const conflicts: Conflict[] = [];
        ...
        for (const coach of teamCoaches) {
          ...
          if (coachTeams) {
            ...
            const { data: coachEvents, error: eventsError } = await supabase
              ...
              .gte(
                "start_time",
                this.getTimeWindow(request.proposedEvent.start, -1)
              )
              .lte(
                "start_time",
                this.getTimeWindow(request.proposedEvent.start, 1)
                ...
                if (
                  ...
                  conflicts.push({
                    ...
                    resolution: {
                      type: "reschedule",
                      suggestedAction:
                        "Coordinate with coach to find available time",
                      automatable: false,
                    },
                  });
                }
   ...
   */
  static async detectVenueConflicts(
    request: ConflictDetectionRequest
  ): Promise<Conflict[]> {
    const conflicts: Conflict[] = [];
    ...
    try {
      ...
      const { data: venueEvents, error } = await supabase
        ...
        .gte(
          "start_time",
          this.getTimeWindow(request.proposedEvent.start, -0.5)
        )
        .lte(
          "start_time",
          this.getTimeWindow(request.proposedEvent.start, 0.5)
  ...
  static async detectAcademicConflicts(
    ...
    try {
      ...
      const academicEvents = this.getAcademicEvents(
        request.proposedEvent.start
      );
        ...
        if (this.isSameDay(eventDate, academicDate)) {
          conflicts.push({
            ...
            conflictingEvent: {
              ...
              start:
                typeof academicEvent.date === "string"
                  ? academicEvent.date
                  : academicEvent.date.toISOString(),
              type: "other",
            } as CalendarEvent,
   ...
   */
  static async detectTravelConflicts(
    request: ConflictDetectionRequest
  ): Promise<Conflict[]> {
    const conflicts: Conflict[] = [];
    ...
    try {
      ...
      if (surroundingEvents && request.proposedEvent.location) {
        for (const event of surroundingEvents) {
          if (
            event.location &&
            event.location !== request.proposedEvent.location
          ) {
   ...
   */
  static calculateOverallSeverity(
    conflicts: Conflict[]
   ...
   */
  static async findSafeTimeSlots(
    request: ConflictDetectionRequest
  ): Promise<Date[]> {
      ...
      for (let hour = 9; hour < 21; hour++) {
        ...
        const testEvent = {
          ...
          start: testDate.toISOString(),
            ...
            testDate.getTime() + duration * 60 * 1000
          ).toISOString(),
        };
