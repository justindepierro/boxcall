...
export class PlayAnalyticsService {
   ...
   */
  static async getGamePlanningAnalytics(teamId: string): Promise<GamePlanningAnalyticsData> {
    try {
      ...
      const metrics = PlayAnalyticsService.calculateGamePlanMetrics(gamePlans || [], situations, plays);
      const insights = PlayAnalyticsService.generateGamePlanInsights(gamePlans || [], situations, plays, analytics);
  ...
  private static async getSituationsData(teamId: string): Promise<GamePlanSituation[]> {
    try {
  ...
  private static async getAnalyticsData(teamId: string): Promise<GamePlanAnalytics[]> {
    try {
      const { data: gamePlans } = await supabase
        .from('game_plans_enhanced')
        .select('id')
  ...
  static async getPlaybookAnalytics(playbookId: string): Promise<PlaybookAnalyticsSummary> {
    ...
    const playAnalytics = plays.map(play => PlayAnalyticsService.calculatePlayAnalytics(play));
    const formationAnalytics = PlayAnalyticsService.calculateFormationAnalytics(plays);
    const situationalPerformance = PlayAnalyticsService.calculateSituationalPerformance(plays);
