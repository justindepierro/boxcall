...
export class PracticeService {
  ...
  static async updatePracticeSchedule(id: string, updates: Partial<PracticeSchedule>): Promise<PracticeSchedule> {
  ...
  static async updatePracticeBlock(scheduleId: string, blockId: string, updates: Partial<PracticeBlock>): Promise<void> {
    const { data: schedule, error: fetchError } = await supabase
      .from('practice_schedules')
      .select('blocks')
      .eq('id', scheduleId)
  ...
  static async deletePracticeBlock(scheduleId: string, blockId: string): Promise<void> {
    const { data: schedule, error: fetchError } = await supabase
      .from('practice_schedules')
      .select('blocks')
      .eq('id', scheduleId)
    ...
    const updatedBlocks = schedule.blocks.filter((block: PracticeBlock) => block.id !== blockId);
  ...
  static async createPracticeTemplate(template: Omit<PracticeTemplate, 'id' | 'createdAt' | 'usageCount'>): Promise<PracticeTemplate> {
    const { data, error } = await supabase
  ...
  static async getPracticeTemplates(teamId: string): Promise<PracticeTemplate[]> {
    const { data, error } = await supabase
      .from('practice_templates')
      .select('*')
  ...
  static async recordAttendance(practiceId: string, playerId: string, status: 'present' | 'absent' | 'late' | 'excused', notes?: string): Promise<PracticeAttendance> {
    const { data, error } = await supabase
  ...
  static async getPracticeAttendance(practiceId: string): Promise<PracticeAttendance[]> {
    const { data, error } = await supabase
      .from('practice_attendance')
      .select('*')
  ...
  static async searchPractices(query: string, teamId: string): Promise<PracticeSearchResult> {
  ...
  private static async searchPracticeSchedules(query: string, teamId: string): Promise<PracticeSchedule[]> {
    const { data, error } = await supabase
      .from('practice_schedules')
      .select('*')
      .eq('team_id', teamId)
  ...
  private static async searchPracticeTemplates(query: string, teamId: string): Promise<PracticeTemplate[]> {
    const { data, error } = await supabase
      .from('practice_templates')
      .select('*')
      .or(`team_id.eq.${teamId},is_public.eq.true`)
  ...
  // Utility Methods
  private static transformScheduleFromDB(dbSchedule: Record<string, unknown>): PracticeSchedule {
    return {
      ...
      fieldType: dbSchedule.field_type as 'indoor' | 'outdoor' | 'gym' | 'field',
  ...
  private static transformTemplateFromDB(dbTemplate: Record<string, unknown>): PracticeTemplate {
    return {
      ...
      defaultFieldType: dbTemplate.default_field_type as 'indoor' | 'outdoor' | 'gym' | 'field',
  ...
  private static transformAttendanceFromDB(dbAttendance: Record<string, unknown>): PracticeAttendance {
    return {
      ...
      arrivalTime: dbAttendance.arrival_time ? new Date(dbAttendance.arrival_time as string) : undefined,
  ...
  private static transformEquipmentFromDB(dbEquipment: Record<string, unknown>): Equipment {
    return {
      id: dbEquipment.id as string,
      name: dbEquipment.name as string,
      category: dbEquipment.category as 'balls' | 'cones' | 'dummies' | 'sleds' | 'protective' | 'other',
      quantity: dbEquipment.quantity as number,
      available: dbEquipment.available as number,
      condition: dbEquipment.condition as 'excellent' | 'good' | 'fair' | 'poor',
      location: dbEquipment.location as string,
      lastChecked: new Date(dbEquipment.last_checked as string)
    };
  }

  private static recalculateBlockTimes(blocks: PracticeBlock[]): PracticeBlock[] {
  ...
  private static calculateBlockTimesFromTemplate(templateBlocks: Record<string, unknown>[], practiceStartTime: Date): PracticeBlock[] {
    let currentTime = new Date(practiceStartTime);

    return templateBlocks.map((templateBlock, index) => {
      const startTime = new Date(currentTime);
      const endTime = new Date(startTime.getTime() + (templateBlock.duration as number) * 60000);
  ...
  private static async searchPracticeScripts(query: string, teamId: string): Promise<PracticeScript[]> {
    ...
    console.log(`Searching practice scripts for query: ${query}, teamId: ${teamId}`);
    return [];
  }
}
