...
export const testSmartIconSystem = () => {
  ...
  testCases.forEach(({ text, expected }, index) => {
    ...
    const isCorrectCategory = SmartIconSystem.getIconSuggestions(text, 5).includes(result);
    
    console.log(`Test ${index + 1}: "${text}"`);
    console.log(`  Expected category: ${expected}`);
    console.log(`  Got: ${result}`);
    console.log(`  Suggestions: ${SmartIconSystem.getIconSuggestions(text, 3).join(', ')}`);
    console.log(`  ✅ Result makes sense: ${isCorrectCategory || result === expected ? 'YES' : 'NO'}`);
    console.log('');
  ...
  const contextTests = [
    { text: "team meeting", context: "calendar" as const, expected: "calendar" },
    { text: "trophy winner", context: "achievement" as const, expected: "trophy" },
  ...
  contextTests.forEach(({ text, context, expected }) => {
    ...
    console.log(`"${text}" in ${context} context: ${result} (expected ${expected})`);
  });
...
export const quickSmartIconTest = () => {
  ...
  examples.forEach(text => {
    ...
    console.log(`"${text}" → ${icon} (alt: ${suggestions.slice(1).join(', ')})`);
  });
};
