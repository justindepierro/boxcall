...
import type { CreatePlayExecutionData, OfflineExecution, OfflineQueue } from "../types/session";
  ...
  async syncQueue(): Promise<number> {
    ...
    for (const offlineExec of pending) {
      try {
        ...
        // Mark as synced
        const index = queue.executions.findIndex((e) => e.id === offlineExec.id);
        if (index !== -1) {
        ...
        const index = queue.executions.findIndex((e) => e.id === offlineExec.id);
        if (index !== -1) {
          queue.executions[index].syncError = err instanceof Error ? err.message : "Unknown error";
        }
      }
    }
