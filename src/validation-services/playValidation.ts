...
export class PlayValidationService {
  ...
  static validatePlay(data: any): ValidationResult {
    ...
    if (!data.play_name || typeof data.play_name !== 'string' || data.play_name.trim() === '') {
      errors.push({
        field: 'play_name',
        code: 'REQUIRED',
    ...
    if (data.f_dir) {
      ...
      if (!validDirs.includes(data.f_dir)) {
        errors.push({
          field: 'f_dir',
          code: 'INVALID_VALUE',
          message: 'Formation direction must be "L", "R", "Left", "Right", "left", or "right"',
          value: data.f_dir
        });
      }
    }
    ...
    if (data.p_dir) {
      ...
      if (!validDirs.includes(data.p_dir)) {
        errors.push({
          field: 'p_dir',
          code: 'INVALID_VALUE',
          message: 'Play direction must be "L", "R", "Left", "Right", "left", or "right"',
          value: data.p_dir
        });
      }
    }
    ...
    if (data.confidence_base !== undefined) {
      if (typeof data.confidence_base !== 'number' || data.confidence_base < 0 || data.confidence_base > 100) {
        errors.push({
      ...
      try {
        ...
        if (formation && data.f_dir) {
          ...
          if (normalizedPlayDir && formationDir) {
            const expectedDir = formationDir === 'left' ? 'L' : formationDir === 'right' ? 'R' : null;
